"""LACE V2 S4 importance-ordered reverse diffusion.

S4 moves from terminal reconstruction probes to a process-level comparison:
does an importance-ordered forward masking schedule create a better reverse
expansion curriculum than random masking with the same token budget?
"""
...
class V2S4Config:
    ...
    output_dir: str = "/kaggle/working/lace_v2_s4"
...
def parse_args() -> V2S4Config:
    parser = argparse.ArgumentParser(description="Run LACE V2 S4 importance-ordered reverse diffusion.")
    parser.add_argument("--model-name", default=V2S4Config.model_name)
    parser.add_argument("--max-train-samples", type=int, default=V2S4Config.max_train_samples)
    parser.add_argument("--max-eval-samples", type=int, default=V2S4Config.max_eval_samples)
    parser.add_argument("--max-length", type=int, default=V2S4Config.max_length)
    parser.add_argument("--target-max-length", type=int, default=V2S4Config.target_max_length)
    parser.add_argument("--skeleton-batch-size", type=int, default=V2S4Config.skeleton_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=V2S4Config.train_batch_size)
    parser.add_argument("--eval-batch-size", type=int, default=V2S4Config.eval_batch_size)
    parser.add_argument("--reverse-epochs", type=int, default=V2S4Config.reverse_epochs)
    parser.add_argument("--learning-rate", type=float, default=V2S4Config.learning_rate)
    parser.add_argument("--d-model", type=int, default=V2S4Config.d_model)
    parser.add_argument("--num-heads", type=int, default=V2S4Config.num_heads)
    parser.add_argument("--encoder-layers", type=int, default=V2S4Config.encoder_layers)
    parser.add_argument("--decoder-layers", type=int, default=V2S4Config.decoder_layers)
    parser.add_argument("--dropout", type=float, default=V2S4Config.dropout)
    parser.add_argument("--max-grad-norm", type=float, default=V2S4Config.max_grad_norm)
    parser.add_argument("--no-repeat-ngram-size", type=int, default=V2S4Config.no_repeat_ngram_size)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", V2S4Config.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=V2S4Config.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=V2S4Config.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=V2S4Config.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=V2S4Config.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=V2S4Config.seed)
    parser.add_argument("--ratios", default=",".join(str(item) for item in DEFAULT_RATIOS))
    parser.add_argument("--gate-tolerance", type=float, default=V2S4Config.gate_tolerance)
    parser.add_argument("--sample-output-count", type=int, default=V2S4Config.sample_output_count)
    parser.add_argument("--min-text-words", type=int, default=V2S4Config.min_text_words)
...
def load_texts(config: V2S4Config) -> tuple[list[str], str]:
...
def build_schedule_payloads(config: V2S4Config, tokenizer, encoder, device: str, texts: list[str]) -> list[dict[str, Any]]:
...
class S4ReverseDiffusionModel(nn.Module):
    def __init__(
        self,
        config: V2S4Config,
        vocab_size: int,
    ...
    def encode(self, input_ids, positions, timesteps, key_padding_mask):
...
def lexical_metrics(prediction: str, target: str, input_text: str, original_text: str) -> dict[str, float]:
    predicted_words = word_tokens(prediction)
    target_words = word_tokens(target)
    input_content = dedupe(content_words(input_text))
    target_content = dedupe(content_words(target))[:12]
    original_content = dedupe(content_words(original_text))[:16]
    expansion_content = [item for item in target_content if item not in set(input_content)]
    ...
    return {
        ...
        "input_retention": (
            sum(1 for item in input_content if item in predicted_set) / len(input_content)
            if input_content
            else 0.0
        ),
        "expansion_recall": (
            sum(1 for item in expansion_content if item in predicted_set) / len(expansion_content)
...
def make_transition_examples(
    ...
    for base in base_examples:
        for transition_index, (from_ratio, to_ratio) in enumerate(zip(ratios[:-1], ratios[1:])):
            ...
            input_ids = list(input_state["ids"])
            positions = list(input_state["positions"])
            if not input_ids:
                input_ids = [tokenizer.pad_token_id]
                positions = [0]
            target_ids = ids_with_eos(list(target_state["ids"]), tokenizer.eos_token_id, config.target_max_length)
            examples.append(
                {
                    ...
                    "target_text": str(target_state["text"]),
    ...
    with torch.no_grad():
        for raw_batch in eval_loader:
            ...
            for item, prediction, generated_ids in zip(raw_batch, predictions, generated.cpu().tolist()):
                metrics = lexical_metrics(prediction, item["target_text"], item["input_text"], item["original_text"])
                rows.append(
                    {
...
def score_for_gate(metrics: dict[str, float]) -> float:
    overlap = metrics.get("token_f1", 0.0) + metrics.get("rouge_l_f1", 0.0)
    semantic = (
        0.25 * metrics.get("target_content_recall", 0.0)
        + 0.25 * metrics.get("input_retention", 0.0)
        + 0.25 * metrics.get("expansion_recall", 0.0)
        + 0.15 * metrics.get("original_content_recall", 0.0)
        + 0.10 * metrics.get("entity_recall", 0.0)
    )
    loss_bonus = 1.0 / (1.0 + max(0.0, metrics.get("eval_loss", 1e9)))
    repetition_penalty = 0.1 * metrics.get("repetition_rate", 0.0)
    return overlap + semantic + loss_bonus - repetition_penalty
...
def evaluate_gates(metrics: dict[str, dict[str, float]], tolerance: float, required_schedules: tuple[str, ...]) -> dict[str, Any]:
    ...
    gates = {
        "S4-G-RUN": {"pass": required.issubset(available), "detail": sorted(available)},
        "S4-G-LOSS-FINITE": {"pass": losses_finite, "detail": {name: item["eval_loss"] for name, item in metrics.items()}},
        "S4-G-IMPORTANCE-BEATS-RANDOM": {
        ...
        "S4-G-IMPORTANCE-BEATS-POSITION-ONLY": {
        ...
        "S4-G-EXPANSION-RECALL": {
            "pass": importance.get("expansion_recall", 0.0) > random_item.get("expansion_recall", 0.0),
            "detail": {
                "importance_expansion_recall": importance.get("expansion_recall"),
                "random_expansion_recall": random_item.get("expansion_recall"),
                "delta": delta(importance.get("expansion_recall"), random_item.get("expansion_recall")),
            },
        },
        "S4-G-RETENTION": {
            "pass": importance.get("input_retention", 0.0) > random_item.get("input_retention", 0.0),
            "detail": {
                "importance_input_retention": importance.get("input_retention"),
                "random_input_retention": random_item.get("input_retention"),
                "delta": delta(importance.get("input_retention"), random_item.get("input_retention")),
            },
        },
        "S4-G-REPETITION": {
            "pass": importance.get("repetition_rate", 1.0) <= random_item.get("repetition_rate", 0.0) + tolerance,
            "detail": {
                "importance_repetition": importance.get("repetition_rate"),
                "random_repetition": random_item.get("repetition_rate"),
                "delta": delta(importance.get("repetition_rate"), random_item.get("repetition_rate")),
                "tolerance": tolerance,
            },
        },
        "S4-G-BEST-IDENTIFIED": {
            "pass": bool(best_name),
    ...
    gates["overall_pass"] = bool(
        gates["S4-G-RUN"]["pass"]
        and gates["S4-G-LOSS-FINITE"]["pass"]
        and gates["S4-G-IMPORTANCE-BEATS-RANDOM"]["pass"]
        and gates["S4-G-IMPORTANCE-BEATS-POSITION-ONLY"]["pass"]
        and gates["S4-G-EXPANSION-RECALL"]["pass"]
        and gates["S4-G-RETENTION"]["pass"]
    )
    gates["process_ready"] = bool(gates["S4-G-RUN"]["pass"] and gates["S4-G-LOSS-FINITE"]["pass"])
...
def render_summary(result: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            f"| `overall_pass` | `{str(gates.get('overall_pass')).lower()}` | Process-level S4 importance schedule signal. |",
            f"| `process_ready` | `{str(gates.get('process_ready')).lower()}` | Whether the schedule comparison ran cleanly. |",
            f"| `s5_ready` | `{str(gates.get('s5_ready')).lower()}` | S4 is constrained transition learning, so this remains false. |",
            "",
            "## Schedule Metrics",
            "",
    ...
    for schedule_name in run_info["schedules"]:
        ...
        lines.append(
            "| `{}` | {:.4f} | {:.2f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} |".format(
                schedule_name,
                item["eval_loss"],
                item["eval_ppl"],
                item["token_f1"],
                item["rouge_l_f1"],
                item["target_content_recall"],
                item["input_retention"],
                item["expansion_recall"],
    ...
    for schedule_name in run_info["schedules"]:
        lines.extend(
            [
                f"### `{schedule_name}`",
                "",
                "| Transition | Loss | Token F1 | ROUGE-L | Target Content | Input Retention | Expansion Recall | Repetition | Score |",
                "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
            ]
        )
    ...
    lines.extend(
        [
            ...
            "",
            "S4 is a constrained reverse-transition experiment, not open-ended generation.",
            "The key comparison is schedule-level reverse curriculum quality, not exact sentence memorization.",
        ]
    )
    return "\n".join(lines) + "\n"


def run(config: V2S4Config) -> dict[str, Any]:
    ...
    result = {
        "run_info": {
            "phase": "v2_s4",
            "experiment_name": "S4-importance ordered reverse diffusion",
