"""LACE V2 S4g pretrained-decoder semantic span expansion.

S4g keeps S4d/S4e's strict gap/span controls but replaces the small custom
reverse decoder with a pretrained text-to-text decoder. The experiment asks
whether language prior reduces span artifacts and improves generated span
content/entity recall without letting no-anchor or wrong-document controls win.
"""
...
class V2S4gConfig:
    ...
    output_dir: str = "/kaggle/working/lace_v2_s4g"
...
def parse_args() -> V2S4gConfig:
    parser = argparse.ArgumentParser(description="Run LACE V2 S4g pretrained-decoder semantic span expansion.")
    parser.add_argument("--model-name", default=V2S4gConfig.model_name)
    parser.add_argument("--max-train-samples", type=int, default=V2S4gConfig.max_train_samples)
    parser.add_argument("--max-eval-samples", type=int, default=V2S4gConfig.max_eval_samples)
    parser.add_argument("--max-length", type=int, default=V2S4gConfig.max_length)
    parser.add_argument("--source-max-length", type=int, default=V2S4gConfig.source_max_length)
    parser.add_argument("--target-max-length", type=int, default=V2S4gConfig.target_max_length)
    parser.add_argument("--generation-max-new-tokens", type=int, default=V2S4gConfig.generation_max_new_tokens)
    parser.add_argument("--max-span-length", type=int, default=V2S4gConfig.max_span_length)
    parser.add_argument("--skeleton-batch-size", type=int, default=V2S4gConfig.skeleton_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=V2S4gConfig.train_batch_size)
    parser.add_argument("--eval-batch-size", type=int, default=V2S4gConfig.eval_batch_size)
    parser.add_argument("--reverse-epochs", type=int, default=V2S4gConfig.reverse_epochs)
    parser.add_argument("--learning-rate", type=float, default=V2S4gConfig.learning_rate)
    parser.add_argument("--use-amp", action=argparse.BooleanOptionalAction, default=V2S4gConfig.use_amp)
    parser.add_argument("--max-grad-norm", type=float, default=V2S4gConfig.max_grad_norm)
    parser.add_argument("--no-repeat-ngram-size", type=int, default=V2S4gConfig.no_repeat_ngram_size)
    parser.add_argument("--num-beams", type=int, default=V2S4gConfig.num_beams)
    parser.add_argument(
        "--max-train-examples-per-schedule",
        type=int,
        default=V2S4gConfig.max_train_examples_per_schedule,
    )
    parser.add_argument(
        "--max-eval-examples-per-schedule",
        type=int,
        default=V2S4gConfig.max_eval_examples_per_schedule,
    )
    parser.add_argument("--rollout-eval-samples", type=int, default=V2S4gConfig.rollout_eval_samples)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", V2S4gConfig.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=V2S4gConfig.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=V2S4gConfig.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=V2S4gConfig.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=V2S4gConfig.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=V2S4gConfig.seed)
    parser.add_argument("--ratios", default=",".join(str(item) for item in DEFAULT_RATIOS))
    parser.add_argument("--gate-tolerance", type=float, default=V2S4gConfig.gate_tolerance)
    parser.add_argument("--sample-output-count", type=int, default=V2S4gConfig.sample_output_count)
    parser.add_argument("--min-text-words", type=int, default=V2S4gConfig.min_text_words)
...
def load_texts(config: V2S4gConfig) -> tuple[list[str], str]:
...
def build_schedule_payloads(config: V2S4gConfig, tokenizer, encoder, device: str, texts: list[str]) -> list[dict[str, Any]]:
...
def collate_examples(batch: list[dict[str, Any]], pad_token_id: int, max_position: int) -> dict[str, Any]:
    ...
    for item in batch:
        ...
        condition_ids.append(int(item.get("condition_id", CONDITION_IDS.get(item.get("schedule", "importance_schedule"), 0))))
...
class S4gSharedSpanReverseModel(nn.Module):
    def __init__(
        self,
        config: V2S4gConfig,
        vocab_size: int,
...
def collate_seq2seq_examples(batch: list[dict[str, Any]], tokenizer, config: V2S4gConfig) -> dict[str, Any]:
...
def evaluate_gates(
    ...
    random_teacher = teacher_metrics.get("random_schedule", {})
    ...
    gates = {
        ...
        "S4G-G-IMPORTANCE-BEATS-POSITION-ONLY": {
        ...
        "S4G-G-IMPORTANCE-BEATS-SAME-POSITION-RANDOM": {
        ...
        "S4G-G-SPAN-CONTENT-GAIN": {
            "pass": importance_teacher.get("delta_content_recall", 0.0)
            > S4E_BASELINES["span_content_recall"] + 0.002,
            "detail": {
                "importance_span_content_recall": importance_teacher.get("delta_content_recall"),
                "s4e_baseline_span_content_recall": S4E_BASELINES["span_content_recall"],
                "s4d_baseline_span_content_recall": S4D_BASELINES["span_content_recall"],
                "delta_vs_s4e": delta(
                    importance_teacher.get("delta_content_recall"),
                    S4E_BASELINES["span_content_recall"],
                ),
                "delta_vs_s4d": delta(importance_teacher.get("delta_content_recall"), S4D_BASELINES["span_content_recall"]),
            },
        },
        "S4G-G-SPAN-ENTITY-GAIN": {
            "pass": importance_teacher.get("entity_recall", 0.0) > S4E_BASELINES["span_entity_recall"] + 0.001,
            "detail": {
                "importance_span_entity_recall": importance_teacher.get("entity_recall"),
                "s4e_baseline_span_entity_recall": S4E_BASELINES["span_entity_recall"],
                "s4d_baseline_span_entity_recall": S4D_BASELINES["span_entity_recall"],
                "delta_vs_s4e": delta(importance_teacher.get("entity_recall"), S4E_BASELINES["span_entity_recall"]),
                "delta_vs_s4d": delta(importance_teacher.get("entity_recall"), S4D_BASELINES["span_entity_recall"]),
            },
        },
        "S4G-G-FINAL-NONREGRESSION": {
            "pass": importance.get("final_content_recall", 0.0) >= S4E_BASELINES["final_content_recall"] - tolerance
            and importance.get("final_entity_recall", 0.0) >= S4E_BASELINES["final_entity_recall"] - tolerance
            and importance.get("semantic_drift_proxy", 1.0) <= S4E_BASELINES["semantic_drift_proxy"] + tolerance
            and importance.get("repetition_rate", 1.0) <= S4E_BASELINES["repetition_rate"] + tolerance,
            "detail": {
                "importance_final_content_recall": importance.get("final_content_recall"),
                "s4e_final_content_recall": S4E_BASELINES["final_content_recall"],
                "s4d_final_content_recall": S4D_BASELINES["final_content_recall"],
                "importance_final_entity_recall": importance.get("final_entity_recall"),
                "s4e_final_entity_recall": S4E_BASELINES["final_entity_recall"],
                "s4d_final_entity_recall": S4D_BASELINES["final_entity_recall"],
                "importance_semantic_drift_proxy": importance.get("semantic_drift_proxy"),
                "s4e_semantic_drift_proxy": S4E_BASELINES["semantic_drift_proxy"],
                "s4d_semantic_drift_proxy": S4D_BASELINES["semantic_drift_proxy"],
                "importance_repetition": importance.get("repetition_rate"),
                "s4e_repetition": S4E_BASELINES["repetition_rate"],
                "s4d_repetition": S4D_BASELINES["repetition_rate"],
                "tolerance": tolerance,
            },
        },
        "S4G-G-ARTIFACT-CHECK": {
            "pass": importance_teacher.get("artifact_rate", 1.0)
            <= S4E_BASELINES["artifact_rate"] - 0.05
            and importance_teacher.get("artifact_rate", 1.0) <= random_teacher.get("artifact_rate", 0.0) + tolerance,
            "detail": {
                "importance_artifact_rate": importance_teacher.get("artifact_rate"),
                "random_artifact_rate": random_teacher.get("artifact_rate"),
                "s4e_artifact_rate": S4E_BASELINES["artifact_rate"],
                "delta": delta(importance_teacher.get("artifact_rate"), random_teacher.get("artifact_rate")),
                "delta_vs_s4e": delta(importance_teacher.get("artifact_rate"), S4E_BASELINES["artifact_rate"]),
                "tolerance": tolerance,
            },
        },
        "S4G-G-ROLLOUT-RANDOM-NONREGRESSION": {
        ...
        "S4G-G-BEST-IDENTIFIED": {
            "pass": bool(best_name),
            "detail": {"best_schedule": best_name, "best_score": scores.get(best_name, 0.0), "scores": scores},
        },
    }
    gates["overall_pass"] = bool(
        gates["S4G-G-RUN"]["pass"]
        and gates["S4G-G-LOSS-FINITE"]["pass"]
        and gates["S4G-G-SHARED-CONDITION-MODEL"]["pass"]
        and gates["S4G-G-PRETRAINED-DECODER"]["pass"]
        and gates["S4G-G-IMPORTANCE-BEATS-RANDOM"]["pass"]
        and gates["S4G-G-IMPORTANCE-BEATS-POSITION-ONLY"]["pass"]
        and gates["S4G-G-IMPORTANCE-BEATS-SAME-POSITION-RANDOM"]["pass"]
        and gates["S4G-G-WRONG-DOC-DROPS"]["pass"]
        and gates["S4G-G-NO-ANCHOR-DROPS"]["pass"]
        and gates["S4G-G-SPAN-CONTENT-GAIN"]["pass"]
        and gates["S4G-G-SPAN-ENTITY-GAIN"]["pass"]
        and gates["S4G-G-FINAL-NONREGRESSION"]["pass"]
        and gates["S4G-G-ARTIFACT-CHECK"]["pass"]
    )
    gates["process_ready"] = bool(gates["S4G-G-RUN"]["pass"] and gates["S4G-G-LOSS-FINITE"]["pass"])
...
def render_summary(result: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"| `s5_ready` | `{str(gates.get('s5_ready')).lower()}` | S4g still uses constrained delta insertion, so this remains false. |",
            "",
            "## Teacher-Forced Step Metrics",
            "",
            "| Schedule | Loss | PPL | TF Delta Acc | Delta F1 | ROUGE-L | Content Recall | Content F1 | Content Rate | Entity Recall | Entity F1 | Artifact | Repetition | Score |",
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for schedule_name in run_info["schedules"]:
        ...
        lines.append(
            "| `{}` | {:.4f} | {:.2f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} |".format(
                schedule_name,
                item["eval_loss"],
                item["eval_ppl"],
    ...
    for schedule_name in run_info["schedules"]:
        lines.extend(
            [
                f"### `{schedule_name}`",
                "",
                "| Transition | Loss | TF Delta Acc | Delta F1 | ROUGE-L | Content Recall | Content F1 | Content Rate | Entity | Artifact | Repetition | Score |",
                "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
            ]
        )
    ...
    lines.extend(
        [
            ...
            "",
            "S4g is a pretrained-decoder gap/span expansion experiment, not open-ended language generation.",
            "The key comparison is whether pretrained language prior improves span content without letting random, same-position, wrong-document, position-only, or no-anchor controls win.",
            "Span content/entity and artifact rates are compared to the S4e baseline because S4g tests whether the S4e failure was mainly a tiny-decoder language-prior bottleneck.",
        ]
    )
    return "\n".join(lines) + "\n"


def run(config: V2S4gConfig) -> dict[str, Any]:
    ...
    result = {
        "run_info": {
            "phase": "v2_s4g",
            "experiment_name": "S4g-pretrained decoder semantic span expansion",
