"""LACE V2 S4b multi-step delta rollout.

S4b keeps the S4a delta-token objective, but changes evaluation from isolated
one-step prediction to an actual 25% -> 50% -> 75% -> 100% reverse rollout.
Each predicted delta is inserted into the target positions and then used as the
next state, so errors can accumulate across the trajectory.
"""
...
class V2S4bConfig:
    ...
    output_dir: str = "/kaggle/working/lace_v2_s4b"
...
def parse_args() -> V2S4bConfig:
    parser = argparse.ArgumentParser(description="Run LACE V2 S4b multi-step delta rollout.")
    parser.add_argument("--model-name", default=V2S4bConfig.model_name)
    parser.add_argument("--max-train-samples", type=int, default=V2S4bConfig.max_train_samples)
    parser.add_argument("--max-eval-samples", type=int, default=V2S4bConfig.max_eval_samples)
    parser.add_argument("--max-length", type=int, default=V2S4bConfig.max_length)
    parser.add_argument("--target-max-length", type=int, default=V2S4bConfig.target_max_length)
    parser.add_argument("--skeleton-batch-size", type=int, default=V2S4bConfig.skeleton_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=V2S4bConfig.train_batch_size)
    parser.add_argument("--eval-batch-size", type=int, default=V2S4bConfig.eval_batch_size)
    parser.add_argument("--reverse-epochs", type=int, default=V2S4bConfig.reverse_epochs)
    parser.add_argument("--learning-rate", type=float, default=V2S4bConfig.learning_rate)
    parser.add_argument("--d-model", type=int, default=V2S4bConfig.d_model)
    parser.add_argument("--num-heads", type=int, default=V2S4bConfig.num_heads)
    parser.add_argument("--encoder-layers", type=int, default=V2S4bConfig.encoder_layers)
    parser.add_argument("--decoder-layers", type=int, default=V2S4bConfig.decoder_layers)
    parser.add_argument("--dropout", type=float, default=V2S4bConfig.dropout)
    parser.add_argument("--max-grad-norm", type=float, default=V2S4bConfig.max_grad_norm)
    parser.add_argument("--no-repeat-ngram-size", type=int, default=V2S4bConfig.no_repeat_ngram_size)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", V2S4bConfig.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=V2S4bConfig.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=V2S4bConfig.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=V2S4bConfig.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=V2S4bConfig.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=V2S4bConfig.seed)
    parser.add_argument("--ratios", default=",".join(str(item) for item in DEFAULT_RATIOS))
    parser.add_argument("--gate-tolerance", type=float, default=V2S4bConfig.gate_tolerance)
    parser.add_argument("--sample-output-count", type=int, default=V2S4bConfig.sample_output_count)
    parser.add_argument("--min-text-words", type=int, default=V2S4bConfig.min_text_words)
...
def load_texts(config: V2S4bConfig) -> tuple[list[str], str]:
...
def build_schedule_payloads(config: V2S4bConfig, tokenizer, encoder, device: str, texts: list[str]) -> list[dict[str, Any]]:
...
class S4bDeltaReverseModel(nn.Module):
    def __init__(
        self,
        config: V2S4bConfig,
        vocab_size: int,
...
def make_rollout_item(
    ...
    if not delta_positions:
        return None
    delta_pairs = [(position, target_position_to_id[position]) for position in delta_positions]
    if schedule_name == "position_only_schedule":
        context_pairs = [(position, int(tokenizer.pad_token_id)) for position in sorted(current_positions)]
    else:
        context_pairs = sorted(current_pairs.items())
    marker_pairs = [(position, int(tokenizer.pad_token_id)) for position, _token_id in delta_pairs]
    encoder_pairs = sorted(context_pairs + marker_pairs, key=lambda item: item[0])
    input_ids = [token_id for _position, token_id in encoder_pairs] or [int(tokenizer.pad_token_id)]
    positions = [position for position, _token_id in encoder_pairs] or [0]
    delta_ids = [token_id for _position, token_id in sorted(delta_pairs, key=lambda item: item[0])]
    return {
        ...
        "input_ids": input_ids,
        "positions": positions,
        "input_text": decode_state_pairs(tokenizer, current_pairs),
        "delta_positions": delta_positions,
        "target_ids": ids_with_eos(delta_ids, tokenizer.eos_token_id, config.target_max_length),
        "target_text": decode_ids(tokenizer, delta_ids),
        "target_state_text": str(target_state["text"]),
...
def evaluate_gates(
    ...
    gates = {
        "S4B-G-RUN": {"pass": required.issubset(available), "detail": sorted(available)},
        "S4B-G-LOSS-FINITE": {
            "pass": losses_finite,
            "detail": {name: item["eval_loss"] for name, item in teacher_metrics.items()},
        },
        "S4B-G-IMPORTANCE-BEATS-RANDOM": {
        ...
        "S4B-G-IMPORTANCE-BEATS-POSITION-ONLY": {
        ...
        "S4B-G-ROLLOUT-SEMANTIC-CONTENT": {
            "pass": importance.get("final_content_recall", 0.0) > random_item.get("final_content_recall", 0.0),
            "detail": {
                "importance_final_content_recall": importance.get("final_content_recall"),
                "random_final_content_recall": random_item.get("final_content_recall"),
                "delta": delta(importance.get("final_content_recall"), random_item.get("final_content_recall")),
            },
        },
        "S4B-G-ROLLOUT-DRIFT-CHECK": {
            ...
            "detail": {
                ...
                "delta": delta(importance.get("semantic_drift_proxy"), random_item.get("semantic_drift_proxy")),
                "tolerance": tolerance,
            },
        },
        "S4B-G-REPETITION-CHECK": {
        ...
        "S4B-G-BEST-IDENTIFIED": {
            "pass": bool(best_name),
            "detail": {"best_schedule": best_name, "best_score": scores.get(best_name, 0.0), "scores": scores},
        },
    }
    gates["overall_pass"] = bool(
        gates["S4B-G-RUN"]["pass"]
        and gates["S4B-G-LOSS-FINITE"]["pass"]
        and gates["S4B-G-IMPORTANCE-BEATS-RANDOM"]["pass"]
        and gates["S4B-G-IMPORTANCE-BEATS-POSITION-ONLY"]["pass"]
        and gates["S4B-G-ROLLOUT-SEMANTIC-CONTENT"]["pass"]
        and gates["S4B-G-ROLLOUT-DRIFT-CHECK"]["pass"]
        and gates["S4B-G-REPETITION-CHECK"]["pass"]
    )
    gates["process_ready"] = bool(gates["S4B-G-RUN"]["pass"] and gates["S4B-G-LOSS-FINITE"]["pass"])
...
def render_summary(result: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"| `s5_ready` | `{str(gates.get('s5_ready')).lower()}` | S4b still uses constrained delta insertion, so this remains false. |",
            "",
    ...
    lines.extend(
        [
            ...
            "",
            "S4b is an actual multi-step reverse rollout, but still a constrained delta-insertion experiment.",
            "The key comparison is whether S4a's one-step importance gain survives error accumulation through the full trajectory.",
        ]
    )
    return "\n".join(lines) + "\n"


def run(config: V2S4bConfig) -> dict[str, Any]:
    ...
    result = {
        "run_info": {
            "phase": "v2_s4b",
            "experiment_name": "S4b-multi-step delta rollout",
