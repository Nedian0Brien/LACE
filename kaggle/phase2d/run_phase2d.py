"""LACE Phase 2D Kaggle runner.

Phase 2D is a bridge experiment before Phase 3A. It does not try to prove the
full compression-forward hypothesis. Instead, it isolates the positional
confounds that Phase 2C exposed so that a later token/generation-aware objective
can be interpreted cleanly.

The key distinction is absolute position versus block-relative position:

    1. absolute position can explain global hidden-state templates,
    2. block-relative position targets the order information average pooling
       destroys inside each compressed block,
    3. shuffled-label diagnostics test whether token proxies are content-aware
       or mostly position-prior driven.
"""
...
class Phase2DConfig:
    ...
    output_dir: str = "/kaggle/working/lace_phase2d"
    ...
    positional_feature_size: int = 64
    wrong_position_shift: int = 1
    wrong_position_shifts: tuple[int, ...] = (1, 2, 4, 8, 16)


Phase2Config = Phase2DConfig
...
def base_condition_name(condition: str) -> str:
    if condition.startswith("average_pool_") and condition.endswith("_pos"):
        return "average_pool"
    if condition == "gaussian_noise_abs_pos":
        return "gaussian_noise"
    if condition.startswith("position_only_") and condition.endswith("_pos"):
        return "average_pool"
    return condition


def condition_uses_positional_encoding(condition: str) -> bool:
    return condition.endswith("_pos")


def condition_position_mode(condition: str) -> str:
    if "abs_rel_pos" in condition:
        return "absolute_relative"
    if "rel_pos" in condition:
        return "relative"
    if "abs_pos" in condition:
        return "absolute"
    return "none"


def condition_is_position_only(condition: str) -> bool:
    return condition.startswith("position_only_")
...
def parse_args() -> Phase2DConfig:
    parser = argparse.ArgumentParser(description="Run LACE Phase 2D positional confound bridge.")
    parser.add_argument("--model-name", default=Phase2DConfig.model_name)
    parser.add_argument("--max-samples", type=int, default=Phase2DConfig.max_samples)
    parser.add_argument("--max-length", type=int, default=Phase2DConfig.max_length)
    parser.add_argument("--encode-batch-size", type=int, default=Phase2DConfig.encode_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=Phase2DConfig.train_batch_size)
    parser.add_argument("--epochs", type=int, default=Phase2DConfig.epochs)
    parser.add_argument("--learning-rate", type=float, default=Phase2DConfig.learning_rate)
    parser.add_argument("--weight-decay", type=float, default=Phase2DConfig.weight_decay)
    parser.add_argument("--val-fraction", type=float, default=Phase2DConfig.val_fraction)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", Phase2DConfig.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=Phase2DConfig.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=Phase2DConfig.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=Phase2DConfig.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=Phase2DConfig.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=Phase2DConfig.seed)
    parser.add_argument("--stage-tokens", default="64,32,16")
    parser.add_argument(
        "--forward-conditions",
        default=(
            "average_pool,average_pool_abs_pos,average_pool_rel_pos,average_pool_abs_rel_pos,"
            "position_only_abs_pos,position_only_rel_pos,position_only_abs_rel_pos,"
            "gaussian_noise,gaussian_noise_abs_pos"
        ),
    )
    ...
    parser.add_argument("--use-matched-gaussian", action=argparse.BooleanOptionalAction, default=Phase2DConfig.use_matched_gaussian)
    parser.add_argument("--lambda-cos", type=float, default=Phase2DConfig.lambda_cos)
    parser.add_argument("--lambda-var", type=float, default=Phase2DConfig.lambda_var)
    parser.add_argument("--perturbation-std", type=float, default=Phase2DConfig.perturbation_std)
    parser.add_argument("--ablation-fraction", type=float, default=Phase2DConfig.ablation_fraction)
    parser.add_argument("--enable-generation-bridge", action=argparse.BooleanOptionalAction, default=Phase2DConfig.enable_generation_bridge)
    parser.add_argument("--generation-eval-batches", type=int, default=Phase2DConfig.generation_eval_batches)
    parser.add_argument("--generation-sample-count", type=int, default=Phase2DConfig.generation_sample_count)
    parser.add_argument("--generation-max-length", type=int, default=Phase2DConfig.generation_max_length)
    parser.add_argument("--enable-token-head-bridge", action=argparse.BooleanOptionalAction, default=Phase2DConfig.enable_token_head_bridge)
    parser.add_argument("--token-head-epochs", type=int, default=Phase2DConfig.token_head_epochs)
    parser.add_argument("--token-head-learning-rate", type=float, default=Phase2DConfig.token_head_learning_rate)
    parser.add_argument("--token-head-train-batches", type=int, default=Phase2DConfig.token_head_train_batches)
    parser.add_argument("--token-head-eval-batches", type=int, default=Phase2DConfig.token_head_eval_batches)
    parser.add_argument("--strict-gate-min-evidence", type=int, default=Phase2DConfig.strict_gate_min_evidence)
    parser.add_argument("--positional-feature-size", type=int, default=Phase2DConfig.positional_feature_size)
    parser.add_argument("--wrong-position-shift", type=int, default=Phase2DConfig.wrong_position_shift)
    parser.add_argument("--wrong-position-shifts", default="1,2,4,8,16")
    ...
    if args.positional_feature_size <= 0 or args.positional_feature_size % 2 != 0:
        raise ValueError("positional-feature-size must be a positive even integer.")
    return Phase2DConfig(
        ...
        positional_feature_size=args.positional_feature_size,
        wrong_position_shift=args.wrong_position_shift,
        wrong_position_shifts=parse_int_tuple(args.wrong_position_shifts),
    )
    ...
    for condition in config.forward_conditions:
        ...
        for stage_index, target_tokens in enumerate(config.stage_tokens, 1):
            ...
            metadata["conditions"][condition][name] = {
                ...
                "position_mode": condition_position_mode(condition),
                "position_only": condition_is_position_only(condition),
                "seed": seed,
            }
            if base_condition == "gaussian_noise":
...
def sinusoidal_features_from_positions(positions, feature_size: int):
    import torch

    if feature_size <= 0 or feature_size % 2 != 0:
        raise ValueError("feature_size must be a positive even integer.")
    positions = positions.float().unsqueeze(1)
    div_term = torch.exp(torch.arange(0, feature_size, 2, dtype=torch.float32) * (-math.log(10000.0) / feature_size))
    features = torch.zeros(positions.shape[0], feature_size, dtype=torch.float32)
    features[:, 0::2] = torch.sin(positions * div_term)
    features[:, 1::2] = torch.cos(positions * div_term)
    return features


def sinusoidal_position_features(max_length: int, feature_size: int):
    import torch

    return sinusoidal_features_from_positions(torch.arange(max_length, dtype=torch.float32), feature_size)


def relative_position_features(max_length: int, target_tokens: int, feature_size: int):
    import torch

    if target_tokens <= 0:
        raise ValueError("target_tokens must be positive.")
    block_size = max(1, math.ceil(max_length / target_tokens))
    positions = torch.arange(max_length, dtype=torch.float32)
    relative_positions = torch.remainder(positions, block_size)
    return sinusoidal_features_from_positions(relative_positions, feature_size)


def build_position_features(max_length: int, target_tokens: int, feature_size: int, mode: str):
    if mode == "absolute":
        return sinusoidal_position_features(max_length, feature_size)
    if mode == "relative":
        return relative_position_features(max_length, target_tokens, feature_size)
    if mode == "absolute_relative":
        half_size = feature_size // 2
        if half_size <= 0 or half_size % 2 != 0:
            raise ValueError("feature_size must be divisible by 4 for absolute_relative mode.")
        absolute = sinusoidal_position_features(max_length, half_size)
        relative = relative_position_features(max_length, target_tokens, half_size)
        return __import__("torch").cat([absolute, relative], dim=-1)
...
def create_expander(
    ...
    class DirectExpander(nn.Module):
        def __init__(self) -> None:
            ...
            self.position_bias = nn.Parameter(torch.zeros(1, max_length, hidden_size))
            if self.use_positional_encoding:
                resolved_target_tokens = int(target_tokens or max_length)
                self.register_buffer(
                    "sinusoidal_position",
                    build_position_features(max_length, resolved_target_tokens, positional_feature_size, position_mode).unsqueeze(0),
                    persistent=False,
                )
                self.position_project = nn.Sequential(
                    nn.LayerNorm(hidden_size + positional_feature_size),
                    nn.Linear(hidden_size + positional_feature_size, hidden_size),
                    nn.GELU(),
        ...
        def forward(self, latents, position_shift: int = 0):
            expanded = interpolate_tokens(latents, max_length)
            if self.use_positional_encoding:
                position = self.sinusoidal_position
                if position_shift:
                    position = position.roll(shifts=position_shift, dims=1)
                position = position.expand(expanded.shape[0], -1, -1).to(expanded.device)
                expanded = self.position_project(torch.cat([expanded, position], dim=-1))
            expanded = expanded + self.position_bias
...
def expander_uses_positional_encoding(expander) -> bool:
    return bool(getattr(expander, "use_positional_encoding", False))
...
def evaluate_stage(
    ...
    wrong_position_delta_values = []
    wrong_position_cosine_delta_values = []
    wrong_position_sweep_values: dict[int, list[float]] = {int(shift): [] for shift in config.wrong_position_shifts}
    shuffled_token_nll_delta_values = []
    shuffled_token_head_nll_delta_values = []
    generation_errors = []
    samples = []
    ...
    with torch.no_grad():
        for latents, hidden_states, attention_mask, input_ids, sample_indices in loader:
            ...
            if expander_uses_positional_encoding(expander):
                for shift in config.wrong_position_shifts:
                    shifted_reconstruction = expander(latents, position_shift=int(shift))
                    shifted_metrics = reconstruction_metrics(hidden_states, shifted_reconstruction, attention_mask)
                    shift_delta_mse = float(shifted_metrics["mse"]) - base_mse
                    wrong_position_sweep_values[int(shift)].append(shift_delta_mse)
                    if int(shift) == int(config.wrong_position_shift):
                        wrong_position_delta_values.append(shift_delta_mse)
                        wrong_position_cosine_delta_values.append(float(shifted_metrics["cosine"]) - base_cosine)
            ...
            if decoder_model is not None and generation_batches_seen < config.generation_eval_batches:
                try:
                    ...
                    shuffled_input_ids = torch.flip(input_ids, dims=(0,))
                    shuffled_token_nll = decoder_nll(decoder_model, reconstructed, attention_mask, shuffled_input_ids)
                    shuffled_token_nll_delta_values.append(float((shuffled_token_nll - token_nll).detach().cpu()))
            ...
            if token_head_model is not None and token_head_batches_seen < config.token_head_eval_batches:
                try:
                    ...
                    shuffled_input_ids = torch.flip(input_ids, dims=(0,))
                    shuffled_head_nll = token_head_nll(token_head_model, reconstructed, attention_mask, shuffled_input_ids)
                    shuffled_token_head_nll_delta_values.append(float((shuffled_head_nll - head_nll).detach().cpu()))
                except Exception as exc:
    ...
    return {
        ...
        "position_diagnostics": {
            "uses_positional_encoding": expander_uses_positional_encoding(expander),
            "wrong_position_shift": config.wrong_position_shift if expander_uses_positional_encoding(expander) else 0,
            "wrong_position_delta_mse": mean(wrong_position_delta_values),
            "wrong_position_delta_cosine": mean(wrong_position_cosine_delta_values),
            "wrong_position_sweep_delta_mse": {str(shift): mean(values) for shift, values in wrong_position_sweep_values.items()},
        },
        "generation_bridge": {
            "token_nll": token_nll,
            ...
            "shuffled_delta_token_nll": mean(shuffled_token_nll_delta_values),
            ...
            "shuffled_delta_token_head_nll": mean(shuffled_token_head_nll_delta_values),
...
def train_condition_stage(
    ...
    use_positional_encoding = condition_uses_positional_encoding(condition)
    position_mode = condition_position_mode(condition)
    expander = create_expander(
        ...
        positional_feature_size=config.positional_feature_size,
        target_tokens=int(latents.shape[1]),
        position_mode=position_mode if use_positional_encoding else "absolute",
    )
...
def max_average_sweep_delta(condition_results: dict[str, Any], shifts: Sequence[int]) -> float:
    values = [
        average_stage_metric(
            condition_results,
            ("final", "position_diagnostics", "wrong_position_sweep_delta_mse", str(shift)),
        )
        for shift in shifts
    ]
...
def evaluate_phase2c_gates(metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    p2c_run = required.issubset(set(results))

    average_pool = results.get("average_pool", {})
    average_pool_positional = results.get("average_pool_positional", {})
    position_only = results.get("position_only", {})
    random_select = results.get("random_select", {})
    gaussian_noise = results.get("gaussian_noise", {})
    gaussian_noise_positional = results.get("gaussian_noise_positional", {})

    avg_pool_mse = average_stage_metric(average_pool, ("final", "mse"))
    avg_pos_mse = average_stage_metric(average_pool_positional, ("final", "mse"))
    pos_only_mse = average_stage_metric(position_only, ("final", "mse"))
    random_mse = average_stage_metric(random_select, ("final", "mse"))
    gaussian_mse = average_stage_metric(gaussian_noise, ("final", "mse"))
    gaussian_pos_mse = average_stage_metric(gaussian_noise_positional, ("final", "mse"))
    p2c_pos_mse = bool(average_pool and average_pool_positional and avg_pos_mse < avg_pool_mse)

    avg_pool_cos = average_stage_metric(average_pool, ("final", "cosine"))
    avg_pos_cos = average_stage_metric(average_pool_positional, ("final", "cosine"))
    pos_only_cos = average_stage_metric(position_only, ("final", "cosine"))
    random_cos = average_stage_metric(random_select, ("final", "cosine"))
    gaussian_cos = average_stage_metric(gaussian_noise, ("final", "cosine"))
    gaussian_pos_cos = average_stage_metric(gaussian_noise_positional, ("final", "cosine"))
    p2c_pos_cos = bool(average_pool and average_pool_positional and avg_pos_cos > avg_pool_cos)

    avg_delta_nll = average_stage_metric(average_pool, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    avg_pos_delta_nll = average_stage_metric(average_pool_positional, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    pos_only_delta_nll = average_stage_metric(position_only, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    ...
    gaussian_pos_delta_nll = average_stage_metric(gaussian_noise_positional, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    p2c_pos_decoder_nll = bool(average_pool and average_pool_positional and avg_pos_delta_nll < avg_delta_nll)

    avg_head_delta = average_stage_metric(average_pool, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    avg_pos_head_delta = average_stage_metric(average_pool_positional, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    pos_only_head_delta = average_stage_metric(position_only, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    ...
    gaussian_pos_head_delta = average_stage_metric(gaussian_noise_positional, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    p2c_pos_token_head = bool(average_pool and average_pool_positional and avg_pos_head_delta < avg_head_delta)

    avg_pos_relative = average_stage_metric(average_pool_positional, ("final", "latent_use", "relative_perturbation_sensitivity"))
    avg_pos_ablation = average_stage_metric(average_pool_positional, ("final", "latent_use", "ablation_delta_mse"))
    avg_pos_swap = average_stage_metric(average_pool_positional, ("final", "latent_use", "swap_delta_mse"))
    pos_only_relative = average_stage_metric(position_only, ("final", "latent_use", "relative_perturbation_sensitivity"))
    pos_only_ablation = average_stage_metric(position_only, ("final", "latent_use", "ablation_delta_mse"))
    pos_only_swap = average_stage_metric(position_only, ("final", "latent_use", "swap_delta_mse"))
    ...
    positional_latent_use_signals = [avg_pos_relative > 0.005, avg_pos_ablation > 0.0005, avg_pos_swap > 0.0005]
    p2c_latent_use = sum(1 for item in positional_latent_use_signals if item) >= 2

    avg_quality = average_stage_metric(average_pool, ("final", "generation_quality", "meaningful_sample_rate"))
    avg_pos_quality = average_stage_metric(average_pool_positional, ("final", "generation_quality", "meaningful_sample_rate"))
    pos_only_quality = average_stage_metric(position_only, ("final", "generation_quality", "meaningful_sample_rate"))
    h0_quality = float(metrics.get("h0_decoder_control", {}).get("generation_quality", {}).get("meaningful_sample_rate", 0.0))
    p2c_generation = bool(avg_pos_quality > avg_quality and avg_pos_quality > 0.0)

    p2c_position_control = bool(
        average_pool_positional
        and position_only
        and avg_pos_mse < pos_only_mse
        and avg_pos_delta_nll < pos_only_delta_nll
        and avg_pos_head_delta < pos_only_head_delta
    )
    wrong_pos_delta_mse = average_stage_metric(average_pool_positional, ("final", "position_diagnostics", "wrong_position_delta_mse"))
    wrong_pos_delta_cos = average_stage_metric(average_pool_positional, ("final", "position_diagnostics", "wrong_position_delta_cosine"))
    p2c_position_matters = bool(average_pool_positional and (wrong_pos_delta_mse > 0.0005 or wrong_pos_delta_cos < -0.0005))

    average_positional_gain_mse = avg_pool_mse - avg_pos_mse
    gaussian_positional_gain_mse = gaussian_mse - gaussian_pos_mse
    average_positional_gain_decoder = avg_delta_nll - avg_pos_delta_nll
    gaussian_positional_gain_decoder = gaussian_delta_nll - gaussian_pos_delta_nll
    ...
    evidence = [
        ...
        p2c_generation,
        p2c_latent_use,
        p2c_position_control,
        p2c_position_matters,
    ]
    min_evidence = int(metrics.get("strict_gate_min_evidence", 2))
    phase3_candidate = p2c_run and sum(1 for item in evidence if item) >= min_evidence and p2c_position_control

    gates = {
        "P2C-G-RUN": {"pass": p2c_run, "reason": "baseline, positional, position-only, random, and matched Gaussian controls ran on the same split"},
        "P2C-G-POS-MSE": {"pass": p2c_pos_mse, "reason": "average_pool_positional improves hidden-state MSE over average_pool"},
        "P2C-G-POS-COS": {"pass": p2c_pos_cos, "reason": "average_pool_positional improves cosine direction over average_pool"},
        "P2C-G-POS-DECODER-NLL": {"pass": p2c_pos_decoder_nll, "reason": "average_pool_positional lowers frozen-decoder delta token NLL"},
        "P2C-G-POS-TOKEN-HEAD-NLL": {"pass": p2c_pos_token_head, "reason": "average_pool_positional lowers lightweight token-head delta NLL"},
        "P2C-G-POS-USE": {"pass": p2c_latent_use, "reason": "positional average-pool expander still responds to latent perturbation, ablation, or swap"},
        "P2C-G-POS-CONTROL": {"pass": p2c_position_control, "reason": "average_pool_positional beats position_only, so any gain is not only a position prior"},
        "P2C-G-POS-MATTERS": {"pass": p2c_position_matters, "reason": "wrong-position evaluation changes reconstruction quality"},
        "P2C-G-GEN": {"pass": p2c_generation, "reason": "average_pool_positional improves meaningful open-ended generation over average_pool"},
    }
    gates["overall_pass"] = all(value["pass"] for key, value in gates.items() if key.startswith("P2C-G-"))
    gates["phase3_candidate"] = phase3_candidate
    gates["diagnostics"] = {
        "avg_pool_mse": avg_pool_mse,
        "avg_pool_positional_mse": avg_pos_mse,
        "position_only_mse": pos_only_mse,
        "random_mse": random_mse,
        "gaussian_mse": gaussian_mse,
        "gaussian_positional_mse": gaussian_pos_mse,
        "avg_pool_cosine": avg_pool_cos,
        "avg_pool_positional_cosine": avg_pos_cos,
        "position_only_cosine": pos_only_cos,
        "random_cosine": random_cos,
        "gaussian_cosine": gaussian_cos,
        "gaussian_positional_cosine": gaussian_pos_cos,
        "avg_delta_token_nll": avg_delta_nll,
        "avg_pool_positional_delta_token_nll": avg_pos_delta_nll,
        "position_only_delta_token_nll": pos_only_delta_nll,
        "random_delta_token_nll": random_delta_nll,
        "gaussian_delta_token_nll": gaussian_delta_nll,
        "gaussian_positional_delta_token_nll": gaussian_pos_delta_nll,
        "avg_delta_token_head_nll": avg_head_delta,
        "avg_pool_positional_delta_token_head_nll": avg_pos_head_delta,
        "position_only_delta_token_head_nll": pos_only_head_delta,
        "random_delta_token_head_nll": random_head_delta,
        "gaussian_delta_token_head_nll": gaussian_head_delta,
        "gaussian_positional_delta_token_head_nll": gaussian_pos_head_delta,
        "avg_relative_sensitivity": avg_relative,
        "avg_ablation_delta": avg_ablation,
        "avg_swap_delta": avg_swap,
        "avg_pool_positional_relative_sensitivity": avg_pos_relative,
        "avg_pool_positional_ablation_delta": avg_pos_ablation,
        "avg_pool_positional_swap_delta": avg_pos_swap,
        "position_only_relative_sensitivity": pos_only_relative,
        "position_only_ablation_delta": pos_only_ablation,
        "position_only_swap_delta": pos_only_swap,
        "avg_meaningful_sample_rate": avg_quality,
        "avg_pool_positional_meaningful_sample_rate": avg_pos_quality,
        "position_only_meaningful_sample_rate": pos_only_quality,
        "h0_meaningful_sample_rate": h0_quality,
        "average_positional_gain_mse": average_positional_gain_mse,
        "gaussian_positional_gain_mse": gaussian_positional_gain_mse,
        "average_positional_gain_decoder_nll": average_positional_gain_decoder,
        "gaussian_positional_gain_decoder_nll": gaussian_positional_gain_decoder,
        "avg_pool_positional_wrong_position_delta_mse": wrong_pos_delta_mse,
        "avg_pool_positional_wrong_position_delta_cosine": wrong_pos_delta_cos,
...
def evaluate_phase2d_gates(metrics: dict[str, Any]) -> dict[str, Any]:
    results = metrics.get("results", {})
    required = {
        "average_pool",
        "average_pool_abs_pos",
    ...
    p2d_run = required.issubset(set(results))

    average_pool = results.get("average_pool", {})
    avg_abs = results.get("average_pool_abs_pos", {})
    avg_rel = results.get("average_pool_rel_pos", {})
    avg_abs_rel = results.get("average_pool_abs_rel_pos", {})
    pos_abs = results.get("position_only_abs_pos", {})
    pos_rel = results.get("position_only_rel_pos", {})
    pos_abs_rel = results.get("position_only_abs_rel_pos", {})
    gaussian = results.get("gaussian_noise", {})
    gaussian_abs = results.get("gaussian_noise_abs_pos", {})

    avg_pool_mse = average_stage_metric(average_pool, ("final", "mse"))
    avg_abs_mse = average_stage_metric(avg_abs, ("final", "mse"))
    avg_rel_mse = average_stage_metric(avg_rel, ("final", "mse"))
    avg_abs_rel_mse = average_stage_metric(avg_abs_rel, ("final", "mse"))
    pos_abs_mse = average_stage_metric(pos_abs, ("final", "mse"))
    pos_rel_mse = average_stage_metric(pos_rel, ("final", "mse"))
    pos_abs_rel_mse = average_stage_metric(pos_abs_rel, ("final", "mse"))
    gaussian_mse = average_stage_metric(gaussian, ("final", "mse"))
    gaussian_abs_mse = average_stage_metric(gaussian_abs, ("final", "mse"))

    avg_pool_cos = average_stage_metric(average_pool, ("final", "cosine"))
    avg_abs_cos = average_stage_metric(avg_abs, ("final", "cosine"))
    avg_rel_cos = average_stage_metric(avg_rel, ("final", "cosine"))
    avg_abs_rel_cos = average_stage_metric(avg_abs_rel, ("final", "cosine"))
    pos_abs_cos = average_stage_metric(pos_abs, ("final", "cosine"))
    pos_rel_cos = average_stage_metric(pos_rel, ("final", "cosine"))
    pos_abs_rel_cos = average_stage_metric(pos_abs_rel, ("final", "cosine"))
    gaussian_cos = average_stage_metric(gaussian, ("final", "cosine"))
    gaussian_abs_cos = average_stage_metric(gaussian_abs, ("final", "cosine"))

    avg_pool_decoder = average_stage_metric(average_pool, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    avg_abs_decoder = average_stage_metric(avg_abs, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    avg_rel_decoder = average_stage_metric(avg_rel, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    avg_abs_rel_decoder = average_stage_metric(avg_abs_rel, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    pos_abs_decoder = average_stage_metric(pos_abs, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    pos_rel_decoder = average_stage_metric(pos_rel, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    pos_abs_rel_decoder = average_stage_metric(pos_abs_rel, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    gaussian_decoder = average_stage_metric(gaussian, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    gaussian_abs_decoder = average_stage_metric(gaussian_abs, ("final", "generation_bridge", "delta_token_nll_vs_h0"))

    avg_pool_head = average_stage_metric(average_pool, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    avg_abs_head = average_stage_metric(avg_abs, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    avg_rel_head = average_stage_metric(avg_rel, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    avg_abs_rel_head = average_stage_metric(avg_abs_rel, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    pos_abs_head = average_stage_metric(pos_abs, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    pos_rel_head = average_stage_metric(pos_rel, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    pos_abs_rel_head = average_stage_metric(pos_abs_rel, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    gaussian_head = average_stage_metric(gaussian, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))
    gaussian_abs_head = average_stage_metric(gaussian_abs, ("final", "generation_bridge", "delta_token_head_nll_vs_h0"))

    avg_abs_rel_shuffled_decoder = average_stage_metric(avg_abs_rel, ("final", "generation_bridge", "shuffled_delta_token_nll"))
    avg_abs_rel_shuffled_head = average_stage_metric(avg_abs_rel, ("final", "generation_bridge", "shuffled_delta_token_head_nll"))
    pos_abs_rel_shuffled_decoder = average_stage_metric(pos_abs_rel, ("final", "generation_bridge", "shuffled_delta_token_nll"))
    pos_abs_rel_shuffled_head = average_stage_metric(pos_abs_rel, ("final", "generation_bridge", "shuffled_delta_token_head_nll"))

    avg_abs_rel_relative = average_stage_metric(avg_abs_rel, ("final", "latent_use", "relative_perturbation_sensitivity"))
    avg_abs_rel_ablation = average_stage_metric(avg_abs_rel, ("final", "latent_use", "ablation_delta_mse"))
    avg_abs_rel_swap = average_stage_metric(avg_abs_rel, ("final", "latent_use", "swap_delta_mse"))
    latent_use = sum(1 for item in [avg_abs_rel_relative > 0.005, avg_abs_rel_ablation > 0.0005, avg_abs_rel_swap > 0.0005]) >= 2

    avg_abs_rel_wrong_shift = average_stage_metric(avg_abs_rel, ("final", "position_diagnostics", "wrong_position_delta_mse"))
    avg_rel_wrong_shift = average_stage_metric(avg_rel, ("final", "position_diagnostics", "wrong_position_delta_mse"))
    raw_wrong_position_shifts = metrics.get("wrong_position_shifts", ())
    wrong_position_shifts = (
        parse_int_tuple(raw_wrong_position_shifts)
        if isinstance(raw_wrong_position_shifts, str)
        else tuple(int(shift) for shift in raw_wrong_position_shifts)
    )
    avg_abs_rel_wrong_sweep_max = max_average_sweep_delta(avg_abs_rel, wrong_position_shifts)
    avg_rel_wrong_sweep_max = max_average_sweep_delta(avg_rel, wrong_position_shifts)

    abs_gain = bool(avg_abs and avg_abs_mse < avg_pool_mse and avg_abs_decoder < avg_pool_decoder)
    rel_gain = bool(avg_rel and avg_rel_mse < avg_pool_mse and avg_rel_decoder < avg_pool_decoder)
    abs_rel_gain = bool(avg_abs_rel and avg_abs_rel_mse < avg_pool_mse and avg_abs_rel_decoder < avg_pool_decoder)
    rel_content = bool(avg_rel and pos_rel and avg_rel_cos > pos_rel_cos and avg_rel_decoder < pos_rel_decoder)
    abs_rel_content = bool(
        avg_abs_rel
        and pos_abs_rel
        and avg_abs_rel_cos > pos_abs_rel_cos
        and avg_abs_rel_decoder < pos_abs_rel_decoder
        and avg_abs_rel_shuffled_head > pos_abs_rel_shuffled_head
    )
    shuffled_content = bool(avg_abs_rel_shuffled_decoder > 0.05 or avg_abs_rel_shuffled_head > 0.05)
    position_matters = bool(max(avg_abs_rel_wrong_shift, avg_rel_wrong_shift, avg_abs_rel_wrong_sweep_max, avg_rel_wrong_sweep_max) > 0.0005)
    gaussian_still_stronger = bool(gaussian_abs and gaussian_abs_decoder < avg_abs_rel_decoder and gaussian_abs_head < avg_abs_rel_head)

    evidence = [abs_gain, rel_gain, abs_rel_gain, rel_content, abs_rel_content, shuffled_content, latent_use, position_matters]
    min_evidence = int(metrics.get("strict_gate_min_evidence", 2))
    phase3_bridge_ready = p2d_run and shuffled_content and position_matters and sum(1 for item in evidence if item) >= min_evidence

    gates = {
        "P2D-G-RUN": {"pass": p2d_run, "reason": "absolute, relative, absolute+relative, position-only, and Gaussian controls ran"},
        "P2D-G-ABS-GAIN": {"pass": abs_gain, "reason": "absolute positional conditioning improves average_pool MSE and decoder delta NLL"},
        "P2D-G-REL-GAIN": {"pass": rel_gain, "reason": "block-relative positional conditioning improves average_pool MSE and decoder delta NLL"},
        "P2D-G-ABSREL-GAIN": {"pass": abs_rel_gain, "reason": "absolute+relative positional conditioning improves average_pool MSE and decoder delta NLL"},
        "P2D-G-REL-CONTENT": {"pass": rel_content, "reason": "relative average_pool beats relative position_only on cosine and decoder delta NLL"},
        "P2D-G-ABSREL-CONTENT": {"pass": abs_rel_content, "reason": "absolute+relative average_pool beats position_only and has stronger shuffled-label sensitivity"},
        "P2D-G-SHUFFLED-LABEL": {"pass": shuffled_content, "reason": "token proxies worsen when labels are shuffled, indicating content sensitivity"},
        "P2D-G-LATENT-USE": {"pass": latent_use, "reason": "absolute+relative average_pool remains sensitive to perturbation, ablation, or swap"},
        "P2D-G-POS-MATTERS": {"pass": position_matters, "reason": "wrong-position sweep changes reconstruction quality across at least one diagnostic shift"},
        "P2D-G-GAUSSIAN-STRONGER": {"pass": gaussian_still_stronger, "reason": "Gaussian absolute positional remains stronger on decoder and token-head proxies"},
    }
    gates["overall_pass"] = all(value["pass"] for key, value in gates.items() if key.startswith("P2D-G-") and key != "P2D-G-GAUSSIAN-STRONGER")
    gates["phase3_bridge_ready"] = phase3_bridge_ready
    gates["diagnostics"] = {
        "avg_pool_mse": avg_pool_mse,
        "avg_abs_mse": avg_abs_mse,
        "avg_rel_mse": avg_rel_mse,
        "avg_abs_rel_mse": avg_abs_rel_mse,
        "pos_abs_mse": pos_abs_mse,
        "pos_rel_mse": pos_rel_mse,
        "pos_abs_rel_mse": pos_abs_rel_mse,
        "gaussian_mse": gaussian_mse,
        "gaussian_abs_mse": gaussian_abs_mse,
        "avg_pool_cosine": avg_pool_cos,
        "avg_abs_cosine": avg_abs_cos,
        "avg_rel_cosine": avg_rel_cos,
        "avg_abs_rel_cosine": avg_abs_rel_cos,
        "pos_abs_cosine": pos_abs_cos,
        "pos_rel_cosine": pos_rel_cos,
        "pos_abs_rel_cosine": pos_abs_rel_cos,
        "gaussian_cosine": gaussian_cos,
        "gaussian_abs_cosine": gaussian_abs_cos,
        "avg_pool_delta_token_nll": avg_pool_decoder,
        "avg_abs_delta_token_nll": avg_abs_decoder,
        "avg_rel_delta_token_nll": avg_rel_decoder,
        "avg_abs_rel_delta_token_nll": avg_abs_rel_decoder,
        "pos_abs_delta_token_nll": pos_abs_decoder,
        "pos_rel_delta_token_nll": pos_rel_decoder,
        "pos_abs_rel_delta_token_nll": pos_abs_rel_decoder,
        "gaussian_delta_token_nll": gaussian_decoder,
        "gaussian_abs_delta_token_nll": gaussian_abs_decoder,
        "avg_pool_delta_token_head_nll": avg_pool_head,
        "avg_abs_delta_token_head_nll": avg_abs_head,
        "avg_rel_delta_token_head_nll": avg_rel_head,
        "avg_abs_rel_delta_token_head_nll": avg_abs_rel_head,
        "pos_abs_delta_token_head_nll": pos_abs_head,
        "pos_rel_delta_token_head_nll": pos_rel_head,
        "pos_abs_rel_delta_token_head_nll": pos_abs_rel_head,
        "gaussian_delta_token_head_nll": gaussian_head,
        "gaussian_abs_delta_token_head_nll": gaussian_abs_head,
        "avg_abs_rel_shuffled_delta_token_nll": avg_abs_rel_shuffled_decoder,
        "avg_abs_rel_shuffled_delta_token_head_nll": avg_abs_rel_shuffled_head,
        "pos_abs_rel_shuffled_delta_token_nll": pos_abs_rel_shuffled_decoder,
        "pos_abs_rel_shuffled_delta_token_head_nll": pos_abs_rel_shuffled_head,
        "avg_abs_rel_relative_sensitivity": avg_abs_rel_relative,
        "avg_abs_rel_ablation_delta": avg_abs_rel_ablation,
        "avg_abs_rel_swap_delta": avg_abs_rel_swap,
        "avg_abs_rel_wrong_position_delta_mse": avg_abs_rel_wrong_shift,
        "avg_rel_wrong_position_delta_mse": avg_rel_wrong_shift,
        "avg_abs_rel_wrong_position_sweep_max_delta_mse": avg_abs_rel_wrong_sweep_max,
        "avg_rel_wrong_position_sweep_max_delta_mse": avg_rel_wrong_sweep_max,
        "evidence_count": sum(1 for item in evidence if item),
        "min_evidence": min_evidence,
    }
    return gates
...
evaluate_phase2_gates = evaluate_phase2d_gates
...
def write_summary(output_dir: Path, metrics: dict[str, Any]) -> None:
    lines = [
        ...
        f"- Positional feature size: `{metrics['positional_feature_size']}`",
        f"- Wrong-position shift: `{metrics['wrong_position_shift']}`",
        f"- Wrong-position shifts: `{metrics['wrong_position_shifts']}`",
        f"- Phase 3 bridge ready: `{metrics['gates']['phase3_bridge_ready']}`",
        "",
        "## Gate Summary",
        "",
        "| Gate | Pass | Reason |",
    ...
    for gate_name, gate in metrics["gates"].items():
        if gate_name in {"overall_pass", "phase3_candidate", "phase3_bridge_ready", "diagnostics"}:
            continue
    ...
    for condition, condition_results in metrics["results"].items():
        lines.extend(
            [
                f"### {condition}",
                "",
                "| Stage | Pos mode | Val loss | MSE | Cosine | Var ratio | Rel sens | Abl delta | Swap delta | Wrong-pos dMSE | Decoder dNLL | Head dNLL | Meaningful gen |",
                "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
            ]
        )
        for stage_name, stage_result in condition_results.items():
            ...
            position_diag = final["position_diagnostics"]
            ...
            lines.append(
                f"| {stage_name} | `{stage_result['position_mode']}` | {stage_result['validation']['final_loss']:.6f} | {final['mse']:.6f} | "
                ...
                f"{position_diag['wrong_position_delta_mse']:.6f} | "
...
def run_phase2d(config: Phase2Config) -> dict[str, Any]:
    ...
    metrics = {
        ...
        "positional_feature_size": config.positional_feature_size,
        "wrong_position_shift": config.wrong_position_shift,
        "wrong_position_shifts": list(config.wrong_position_shifts),
    ...
    metrics["gates"] = evaluate_phase2d_gates(metrics)
...
run_phase2 = run_phase2d


def main() -> None:
    config = parse_args()
    metrics = run_phase2d(config)
