"""LACE Phase 3A Kaggle runner.

Phase 3A moves beyond reconstruction-only reverse expansion. It keeps the
Phase 2D positional controls, then adds a frozen lightweight token-head loss to
the expander objective so we can test whether token/generation readability
improves without losing latent use.

The key distinction is source condition versus objective arm:

    1. source condition decides which latent is expanded,
    2. objective suffix decides how much frozen token-head loss is used,
    3. position-only and Gaussian arms stay in the run as shortcut controls.
"""
...
class Phase3AConfig:
    ...
    output_dir: str = "/kaggle/working/lace_phase3a"
    ...
    forward_conditions: tuple[str, ...] = (
        ...
        "average_pool_rel_pos_tok005",
        "average_pool_rel_pos_tok010",
        "average_pool_rel_pos_tok020",
        "average_pool_abs_rel_pos_tok010",
        "position_only_rel_pos_tok010",
        "position_only_abs_rel_pos_tok010",
...
Phase3Config = Phase3AConfig


OBJECTIVE_SUFFIX_TO_LAMBDA = {
    "recon": 0.0,
    "tok005": 0.05,
    "tok010": 0.10,
    "tok020": 0.20,
...
def parse_forward_conditions(value: str) -> tuple[str, ...]:
    ...
    invalid = [condition for condition in conditions if source_condition_name(condition) not in BASE_FORWARD_CONDITIONS]
    if invalid:
        raise ValueError(f"Invalid forward conditions: {invalid}")
    return conditions or ("average_pool",)


def split_objective_suffix(condition: str) -> tuple[str, str | None]:
    for suffix in OBJECTIVE_SUFFIX_TO_LAMBDA:
        marker = f"_{suffix}"
        if condition.endswith(marker):
            return condition.removesuffix(marker), suffix
    return condition, None


def source_condition_name(condition: str) -> str:
    source_condition, _suffix = split_objective_suffix(condition)
    return source_condition


def condition_token_loss_weight(condition: str, config: Phase3Config) -> float:
    _source_condition, suffix = split_objective_suffix(condition)
    if suffix is None:
        return float(config.lambda_token)
    return float(OBJECTIVE_SUFFIX_TO_LAMBDA[suffix])


def base_condition_name(condition: str) -> str:
    condition = source_condition_name(condition)
...
def parse_args() -> Phase3AConfig:
    parser = argparse.ArgumentParser(description="Run LACE Phase 3A generation-aware reverse objective.")
    parser.add_argument("--model-name", default=Phase3AConfig.model_name)
    parser.add_argument("--max-samples", type=int, default=Phase3AConfig.max_samples)
    parser.add_argument("--max-length", type=int, default=Phase3AConfig.max_length)
    parser.add_argument("--encode-batch-size", type=int, default=Phase3AConfig.encode_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=Phase3AConfig.train_batch_size)
    parser.add_argument("--epochs", type=int, default=Phase3AConfig.epochs)
    parser.add_argument("--learning-rate", type=float, default=Phase3AConfig.learning_rate)
    parser.add_argument("--weight-decay", type=float, default=Phase3AConfig.weight_decay)
    parser.add_argument("--val-fraction", type=float, default=Phase3AConfig.val_fraction)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", Phase3AConfig.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=Phase3AConfig.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=Phase3AConfig.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=Phase3AConfig.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=Phase3AConfig.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=Phase3AConfig.seed)
    parser.add_argument("--stage-tokens", default="64,32,16")
    parser.add_argument(
        "--forward-conditions",
        default=",".join(Phase3AConfig.forward_conditions),
    )
    ...
    parser.add_argument("--use-matched-gaussian", action=argparse.BooleanOptionalAction, default=Phase3AConfig.use_matched_gaussian)
    parser.add_argument("--lambda-cos", type=float, default=Phase3AConfig.lambda_cos)
    parser.add_argument("--lambda-var", type=float, default=Phase3AConfig.lambda_var)
    parser.add_argument("--lambda-token", type=float, default=Phase3AConfig.lambda_token)
    parser.add_argument("--perturbation-std", type=float, default=Phase3AConfig.perturbation_std)
    parser.add_argument("--ablation-fraction", type=float, default=Phase3AConfig.ablation_fraction)
    parser.add_argument("--enable-generation-bridge", action=argparse.BooleanOptionalAction, default=Phase3AConfig.enable_generation_bridge)
    parser.add_argument("--generation-eval-batches", type=int, default=Phase3AConfig.generation_eval_batches)
    parser.add_argument("--generation-sample-count", type=int, default=Phase3AConfig.generation_sample_count)
    parser.add_argument("--generation-max-length", type=int, default=Phase3AConfig.generation_max_length)
    parser.add_argument("--enable-token-head-bridge", action=argparse.BooleanOptionalAction, default=Phase3AConfig.enable_token_head_bridge)
    parser.add_argument("--token-head-epochs", type=int, default=Phase3AConfig.token_head_epochs)
    parser.add_argument("--token-head-learning-rate", type=float, default=Phase3AConfig.token_head_learning_rate)
    parser.add_argument("--token-head-train-batches", type=int, default=Phase3AConfig.token_head_train_batches)
    parser.add_argument("--token-head-eval-batches", type=int, default=Phase3AConfig.token_head_eval_batches)
    parser.add_argument("--strict-gate-min-evidence", type=int, default=Phase3AConfig.strict_gate_min_evidence)
    parser.add_argument("--positional-feature-size", type=int, default=Phase3AConfig.positional_feature_size)
    parser.add_argument("--wrong-position-shift", type=int, default=Phase3AConfig.wrong_position_shift)
...
def load_hf_dataset_texts(config: Phase3Config) -> list[str]:
    try:
...
def load_texts(config: Phase3Config) -> tuple[list[str], str]:
    source = "kaggle_input"
...
def encode_texts(config: Phase3Config, texts: list[str]):
    import torch
...
def build_forward_cache(hidden_states, attention_mask, config: Phase3Config) -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    del attention_mask
    import torch
    import torch.nn.functional as functional
    ...
    for condition in config.forward_conditions:
        ...
        for stage_index, target_tokens in enumerate(config.stage_tokens, 1):
            ...
            metadata["conditions"][condition][name] = {
                "stage": name,
                "source_condition": source_condition_name(condition),
                ...
                "lambda_token": condition_token_loss_weight(condition, config),
                "seed": seed,
            }
...
def expander_loss(
    ...
    token = reconstructed.new_tensor(0.0)
    if token_head_model is not None and input_ids is not None and lambda_token > 0:
        token = token_head_nll(token_head_model, reconstructed, attention_mask, input_ids)
    loss = mse + (config.lambda_cos * cos) + (config.lambda_var * var) + (float(lambda_token) * token)
    return loss, {
        "mse": float(mse.detach().cpu()),
        "cosine_distance": float(cos.detach().cpu()),
        "variance_penalty": float(var.detach().cpu()),
        "token_head_loss": float(token.detach().cpu()),
        "lambda_token": float(lambda_token),
    }
...
def train_token_head_bridge(config: Phase3Config, hidden_states, attention_mask, input_ids, tokenizer, device: str):
    ...
    token_head_parameter_count = sum(parameter.numel() for parameter in token_head.parameters())
    token_head.eval()
    for parameter in token_head.parameters():
        parameter.requires_grad_(False)
    return token_head, {
        "enabled": True,
        "parameter_count": token_head_parameter_count,
...
def maybe_decode_samples(decoder_model, tokenizer, encoder_hidden, attention_mask, sample_indices, texts: list[str], config: Phase3Config):
    from transformers.modeling_outputs import BaseModelOutput
...
def train_condition_stage(
    ...
    initial_validation = evaluate_stage(expander, val_loader, config, device, token_head_model=token_head_model, lambda_token=lambda_token)
    ...
    for epoch in range(1, config.epochs + 1):
        expander.train()
        epoch_losses = []
        for batch_latents, batch_hidden, batch_mask, batch_input_ids, _sample_indices in train_loader:
        ...
        validation = evaluate_stage(expander, val_loader, config, device, token_head_model=token_head_model, lambda_token=lambda_token)
...
def prepare_generation_bridge(config: Phase3Config, device: str):
...
def save_cache(output_dir: Path, texts: list[str], text_source: str, hidden_states, attention_mask, input_ids, config: Phase3Config) -> tuple[Path, bool]:
    import torch
...
def evaluate_phase3a_gates(metrics: dict[str, Any]) -> dict[str, Any]:
    results = metrics.get("results", {})
    required = {
        "average_pool_rel_pos_recon",
        "average_pool_rel_pos_tok005",
    ...
    p3a_run = required.issubset(set(results))

    def condition_metrics(condition: str) -> dict[str, float]:
        condition_results = results.get(condition, {})
        return {
            "mse": average_stage_metric(condition_results, ("final", "mse")),
            "cosine": average_stage_metric(condition_results, ("final", "cosine")),
            "decoder_delta": average_stage_metric(condition_results, ("final", "generation_bridge", "delta_token_nll_vs_h0")),
            "head_delta": average_stage_metric(condition_results, ("final", "generation_bridge", "delta_token_head_nll_vs_h0")),
            "shuffled_decoder_delta": average_stage_metric(condition_results, ("final", "generation_bridge", "shuffled_delta_token_nll")),
            "shuffled_head_delta": average_stage_metric(condition_results, ("final", "generation_bridge", "shuffled_delta_token_head_nll")),
            "meaningful_generation": average_stage_metric(condition_results, ("final", "generation_quality", "meaningful_sample_rate")),
            "relative_sensitivity": average_stage_metric(condition_results, ("final", "latent_use", "relative_perturbation_sensitivity")),
            "ablation_delta": average_stage_metric(condition_results, ("final", "latent_use", "ablation_delta_mse")),
            "swap_delta": average_stage_metric(condition_results, ("final", "latent_use", "swap_delta_mse")),
        }

    diagnostics_by_condition = {condition: condition_metrics(condition) for condition in results}
    rel_recon_name = "average_pool_rel_pos_recon"
    rel_token_names = ["average_pool_rel_pos_tok005", "average_pool_rel_pos_tok010", "average_pool_rel_pos_tok020"]
    available_rel_token_names = [name for name in rel_token_names if name in results]
    best_rel_name = min(available_rel_token_names, key=lambda name: diagnostics_by_condition[name]["head_delta"]) if available_rel_token_names else ""
    best_rel = diagnostics_by_condition.get(best_rel_name, {})
    rel_recon = diagnostics_by_condition.get(rel_recon_name, {})
    pos_rel = diagnostics_by_condition.get("position_only_rel_pos_tok010", {})
    pos_abs_rel = diagnostics_by_condition.get("position_only_abs_rel_pos_tok010", {})
    gaussian_recon = diagnostics_by_condition.get("gaussian_noise_abs_pos_recon", {})
    gaussian_token = diagnostics_by_condition.get("gaussian_noise_abs_pos_tok010", {})

    token_head_improves = bool(best_rel and rel_recon and best_rel["head_delta"] < rel_recon["head_delta"])
    decoder_not_worse = bool(best_rel and rel_recon and best_rel["decoder_delta"] <= rel_recon["decoder_delta"] + 0.25)
    latent_use = bool(
        best_rel
        and sum(
            1
            for item in [
                best_rel["relative_sensitivity"] > 0.005,
                best_rel["ablation_delta"] > 0.0005,
                best_rel["swap_delta"] > 0.0005,
            ]
            if item
        )
        >= 2
    )
    content_control = bool(
        best_rel
        and pos_rel
        and pos_abs_rel
        and best_rel["cosine"] > pos_rel["cosine"]
        and best_rel["cosine"] > pos_abs_rel["cosine"]
        and best_rel["shuffled_head_delta"] > pos_rel["shuffled_head_delta"]
    )
    generation_improves = bool(
        best_rel
        and rel_recon
        and best_rel["meaningful_generation"] > rel_recon["meaningful_generation"]
        and best_rel["meaningful_generation"] > 0.0
    )
    gaussian_gap_narrows = bool(
        best_rel
        and rel_recon
        and gaussian_recon
        and (rel_recon["head_delta"] - best_rel["head_delta"]) > 0
        and (best_rel["head_delta"] - gaussian_recon["head_delta"]) < (rel_recon["head_delta"] - gaussian_recon["head_delta"])
    )
    gaussian_token_still_stronger = bool(
        best_rel
        and gaussian_token
        and gaussian_token["decoder_delta"] < best_rel["decoder_delta"]
        and gaussian_token["head_delta"] < best_rel["head_delta"]
    )

    evidence = [token_head_improves, decoder_not_worse, latent_use, content_control, generation_improves]
    min_evidence = int(metrics.get("strict_gate_min_evidence", 2))
    phase3a_success = p3a_run and token_head_improves and latent_use and content_control and generation_improves

    gates = {
        "P3A-G-RUN": {"pass": p3a_run, "reason": "reconstruction, token-loss sweep, position-only, and Gaussian controls ran"},
        "P3A-G-TOKEN-HEAD": {"pass": token_head_improves, "reason": "best Average Pooling token arm lowers token-head delta NLL versus reconstruction-only baseline"},
        "P3A-G-DECODER": {"pass": decoder_not_worse, "reason": "best Average Pooling token arm does not materially worsen frozen decoder delta NLL"},
        "P3A-G-LATENT-USE": {"pass": latent_use, "reason": "best Average Pooling token arm remains sensitive to perturbation, ablation, or swap"},
        "P3A-G-CONTENT-CONTROL": {"pass": content_control, "reason": "best Average Pooling token arm remains more content-sensitive than position-only controls"},
        "P3A-G-GENERATION": {"pass": generation_improves, "reason": "best Average Pooling token arm improves meaningful generation versus reconstruction-only baseline"},
        "P3A-G-GAUSSIAN-GAP": {"pass": gaussian_gap_narrows and not gaussian_token_still_stronger, "reason": "Average Pooling token objective narrows the Gaussian control gap without Gaussian remaining stronger"},
    }
    gates["overall_pass"] = all(value["pass"] for key, value in gates.items() if key.startswith("P3A-G-") and key != "P3A-G-GAUSSIAN-GAP")
    gates["phase3a_success"] = phase3a_success
    gates["diagnostics"] = {
        "best_average_pool_rel_token_arm": best_rel_name,
        "conditions": diagnostics_by_condition,
        "token_head_improvement": rel_recon.get("head_delta", 0.0) - best_rel.get("head_delta", 0.0) if best_rel and rel_recon else 0.0,
        "decoder_delta_change": best_rel.get("decoder_delta", 0.0) - rel_recon.get("decoder_delta", 0.0) if best_rel and rel_recon else 0.0,
        "generation_delta": best_rel.get("meaningful_generation", 0.0) - rel_recon.get("meaningful_generation", 0.0) if best_rel and rel_recon else 0.0,
        "gaussian_token_still_stronger": gaussian_token_still_stronger,
        "evidence_count": sum(1 for item in evidence if item),
        "min_evidence": min_evidence,
    }
    return gates
...
evaluate_phase3_gates = evaluate_phase3a_gates
...
def write_summary(output_dir: Path, metrics: dict[str, Any]) -> None:
    lines = [
        ...
        f"- Phase 3A success: `{metrics['gates']['phase3a_success']}`",
        f"- Best Average Pooling token arm: `{metrics['gates']['diagnostics']['best_average_pool_rel_token_arm']}`",
        "",
        "## Gate Summary",
        "",
        "| Gate | Pass | Reason |",
    ...
    for gate_name, gate in metrics["gates"].items():
        if gate_name in {"overall_pass", "phase3_candidate", "phase3_bridge_ready", "phase3a_success", "diagnostics"}:
            continue
    ...
    for condition, condition_results in metrics["results"].items():
        lines.extend(
            [
                f"### {condition}",
                "",
                "| Stage | Pos mode | Lambda token | Val loss | MSE | Cosine | Var ratio | Rel sens | Abl delta | Swap delta | Wrong-pos dMSE | Decoder dNLL | Head dNLL | Meaningful gen |",
                "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
            ]
        )
        for stage_name, stage_result in condition_results.items():
            ...
            lines.append(
                f"| {stage_name} | `{stage_result['position_mode']}` | {stage_result['lambda_token']:.3f} | {stage_result['validation']['final_loss']:.6f} | {final['mse']:.6f} | "
...
def run_phase3a(config: Phase3Config) -> dict[str, Any]:
    ...
    metrics = {
        ...
        "lambda_token_default": config.lambda_token,
        "positional_feature_size": config.positional_feature_size,
    ...
    metrics["gates"] = evaluate_phase3a_gates(metrics)
...
run_phase2 = run_phase3a


def main() -> None:
    config = parse_args()
    metrics = run_phase3a(config)
