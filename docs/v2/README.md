...
현재 다음 단계는 open-ended generation이 아니다. S5는 "올바른 semantic plan이 있으면 span 생성이 살아난다"는 upper-bound는 확인했지만, "모델이 skeleton/anchor에서 그 plan을 스스로 찾는다"는 것은 확인하지 못했다. 따라서 다음은 S5 내부에서 heuristic planner를 learned semantic planner로 교체하고, predicted plan rollout이 no-plan/random/wrong-document plan보다 좋아지는지 확인하는 것이다.
