"""LACE V2 S4a delta-token reverse objective.

S4a keeps the S4 schedule comparison, but changes the reverse objective.
Instead of regenerating the whole next state, the model predicts only the
tokens that newly appear in the next reverse step.
"""
...
class V2S4aConfig:
    ...
    output_dir: str = "/kaggle/working/lace_v2_s4a"
...
def parse_args() -> V2S4aConfig:
    parser = argparse.ArgumentParser(description="Run LACE V2 S4a delta-token reverse objective.")
    parser.add_argument("--model-name", default=V2S4aConfig.model_name)
    parser.add_argument("--max-train-samples", type=int, default=V2S4aConfig.max_train_samples)
    parser.add_argument("--max-eval-samples", type=int, default=V2S4aConfig.max_eval_samples)
    parser.add_argument("--max-length", type=int, default=V2S4aConfig.max_length)
    parser.add_argument("--target-max-length", type=int, default=V2S4aConfig.target_max_length)
    parser.add_argument("--skeleton-batch-size", type=int, default=V2S4aConfig.skeleton_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=V2S4aConfig.train_batch_size)
    parser.add_argument("--eval-batch-size", type=int, default=V2S4aConfig.eval_batch_size)
    parser.add_argument("--reverse-epochs", type=int, default=V2S4aConfig.reverse_epochs)
    parser.add_argument("--learning-rate", type=float, default=V2S4aConfig.learning_rate)
    parser.add_argument("--d-model", type=int, default=V2S4aConfig.d_model)
    parser.add_argument("--num-heads", type=int, default=V2S4aConfig.num_heads)
    parser.add_argument("--encoder-layers", type=int, default=V2S4aConfig.encoder_layers)
    parser.add_argument("--decoder-layers", type=int, default=V2S4aConfig.decoder_layers)
    parser.add_argument("--dropout", type=float, default=V2S4aConfig.dropout)
    parser.add_argument("--max-grad-norm", type=float, default=V2S4aConfig.max_grad_norm)
    parser.add_argument("--no-repeat-ngram-size", type=int, default=V2S4aConfig.no_repeat_ngram_size)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", V2S4aConfig.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=V2S4aConfig.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=V2S4aConfig.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=V2S4aConfig.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=V2S4aConfig.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=V2S4aConfig.seed)
    parser.add_argument("--ratios", default=",".join(str(item) for item in DEFAULT_RATIOS))
    parser.add_argument("--gate-tolerance", type=float, default=V2S4aConfig.gate_tolerance)
    parser.add_argument("--sample-output-count", type=int, default=V2S4aConfig.sample_output_count)
    parser.add_argument("--min-text-words", type=int, default=V2S4aConfig.min_text_words)
...
def load_texts(config: V2S4aConfig) -> tuple[list[str], str]:
...
def build_schedule_payloads(config: V2S4aConfig, tokenizer, encoder, device: str, texts: list[str]) -> list[dict[str, Any]]:
...
class S4aDeltaReverseModel(nn.Module):
    def __init__(
        self,
        config: V2S4aConfig,
        vocab_size: int,
    ...
    for base in base_examples:
        for transition_index, (from_ratio, to_ratio) in enumerate(zip(ratios[:-1], ratios[1:])):
            ...
            if not delta_pairs:
                continue
            context_pairs = [
                (int(position), int(token_id)) for position, token_id in zip(input_state["positions"], input_state["ids"])
            ]
            marker_pairs = [(position, int(tokenizer.pad_token_id)) for position, _token_id in delta_pairs]
            encoder_pairs = sorted(context_pairs + marker_pairs, key=lambda item: item[0])
            input_ids = [token_id for _position, token_id in encoder_pairs]
            positions = [position for position, _token_id in encoder_pairs]
            if not input_ids:
                input_ids = [tokenizer.pad_token_id]
                positions = [0]
            delta_ids = [token_id for _position, token_id in sorted(delta_pairs, key=lambda item: item[0])]
            target_ids = ids_with_eos(delta_ids, tokenizer.eos_token_id, config.target_max_length)
            context_text = str(context_state["text"])
            examples.append(
                {
                    ...
                    "target_text": decode_ids(tokenizer, delta_ids),
...
def evaluate_gates(metrics: dict[str, dict[str, float]], tolerance: float, required_schedules: tuple[str, ...]) -> dict[str, Any]:
    ...
    gates = {
        "S4A-G-RUN": {"pass": required.issubset(available), "detail": sorted(available)},
        "S4A-G-LOSS-FINITE": {"pass": losses_finite, "detail": {name: item["eval_loss"] for name, item in metrics.items()}},
        "S4A-G-IMPORTANCE-BEATS-RANDOM": {
        ...
        "S4A-G-IMPORTANCE-BEATS-POSITION-ONLY": {
        ...
        "S4A-G-DELTA-CONTENT": {
            "pass": importance.get("delta_content_recall", 0.0) > random_item.get("delta_content_recall", 0.0),
            "detail": {
                "importance_delta_content_recall": importance.get("delta_content_recall"),
                "random_delta_content_recall": random_item.get("delta_content_recall"),
                "delta": delta(importance.get("delta_content_recall"), random_item.get("delta_content_recall")),
            },
        },
        "S4A-G-DELTA-ACCURACY": {
            "pass": importance.get("tf_delta_accuracy", 0.0) > random_item.get("tf_delta_accuracy", 0.0),
            "detail": {
                "importance_tf_delta_accuracy": importance.get("tf_delta_accuracy"),
                "random_tf_delta_accuracy": random_item.get("tf_delta_accuracy"),
                "delta": delta(importance.get("tf_delta_accuracy"), random_item.get("tf_delta_accuracy")),
            },
        },
        "S4A-G-ENTITY-DELTA": {
            "pass": importance.get("entity_recall", 0.0) > random_item.get("entity_recall", 0.0),
            "detail": {
                "importance_entity_recall": importance.get("entity_recall"),
                "random_entity_recall": random_item.get("entity_recall"),
                "delta": delta(importance.get("entity_recall"), random_item.get("entity_recall")),
            },
        },
        "S4A-G-COPY-LEAKAGE": {
            "pass": importance.get("context_copy_rate", 1.0) <= random_item.get("context_copy_rate", 0.0) + tolerance,
            "detail": {
                "importance_context_copy_rate": importance.get("context_copy_rate"),
                "random_context_copy_rate": random_item.get("context_copy_rate"),
                "delta": delta(importance.get("context_copy_rate"), random_item.get("context_copy_rate")),
                "tolerance": tolerance,
            },
        },
        "S4A-G-REPETITION": {
        ...
        "S4A-G-BEST-IDENTIFIED": {
            "pass": bool(best_name),
            "detail": {"best_schedule": best_name, "best_score": scores.get(best_name, 0.0), "scores": scores},
        },
    }
    gates["overall_pass"] = bool(
        gates["S4A-G-RUN"]["pass"]
        and gates["S4A-G-LOSS-FINITE"]["pass"]
        and gates["S4A-G-IMPORTANCE-BEATS-RANDOM"]["pass"]
        and gates["S4A-G-IMPORTANCE-BEATS-POSITION-ONLY"]["pass"]
        and gates["S4A-G-DELTA-ACCURACY"]["pass"]
        and gates["S4A-G-DELTA-CONTENT"]["pass"]
    )
    gates["process_ready"] = bool(gates["S4A-G-RUN"]["pass"] and gates["S4A-G-LOSS-FINITE"]["pass"])
...
def render_summary(result: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            f"| `overall_pass` | `{str(gates.get('overall_pass')).lower()}` | Delta-token objective signal. |",
            ...
            f"| `s5_ready` | `{str(gates.get('s5_ready')).lower()}` | S4a is still constrained delta prediction, so this remains false. |",
            "",
            "## Schedule Metrics",
            "",
    ...
    lines.extend(
        [
            ...
            "",
            "S4a is a constrained delta-token prediction experiment, not open-ended generation.",
            "The key comparison is whether a semantic skeleton helps predict newly unmasked tokens beyond random or position-only controls.",
        ]
    )
    return "\n".join(lines) + "\n"


def run(config: V2S4aConfig) -> dict[str, Any]:
    ...
    result = {
        "run_info": {
            "phase": "v2_s4a",
            "experiment_name": "S4a-delta token reverse objective",
