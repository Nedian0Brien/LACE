"""LACE Phase 2 Kaggle runner.

Phase 2 isolates the forward process question:

    Is a compression-based forward process better than corruption/noise for
    learning the same reverse expansion task?

The script compares fixed compression and corruption conditions under the same
frozen encoder, data split, expander family, optimizer budget, and metrics.
"""
...
class Phase2Config:
    ...
    output_dir: str = "/kaggle/working/lace_phase2"
    ...
    sigma_values: tuple[float, ...] = (0.10, 0.20, 0.40)
    calibration_sigmas: tuple[float, ...] = (0.05, 0.10, 0.20, 0.40, 0.80)
...
def parse_args() -> Phase2Config:
    parser = argparse.ArgumentParser(description="Run LACE Phase 2 forward process isolation.")
    parser.add_argument("--model-name", default=Phase2Config.model_name)
    parser.add_argument("--max-samples", type=int, default=Phase2Config.max_samples)
    parser.add_argument("--max-length", type=int, default=Phase2Config.max_length)
    parser.add_argument("--encode-batch-size", type=int, default=Phase2Config.encode_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=Phase2Config.train_batch_size)
    parser.add_argument("--epochs", type=int, default=Phase2Config.epochs)
    parser.add_argument("--learning-rate", type=float, default=Phase2Config.learning_rate)
    parser.add_argument("--weight-decay", type=float, default=Phase2Config.weight_decay)
    parser.add_argument("--val-fraction", type=float, default=Phase2Config.val_fraction)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", Phase2Config.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=Phase2Config.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=Phase2Config.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=Phase2Config.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=Phase2Config.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=Phase2Config.seed)
    ...
    parser.add_argument("--sigma-values", default="0.10,0.20,0.40")
    parser.add_argument("--calibration-sigmas", default="0.05,0.10,0.20,0.40,0.80")
    parser.add_argument("--lambda-cos", type=float, default=Phase2Config.lambda_cos)
    parser.add_argument("--lambda-var", type=float, default=Phase2Config.lambda_var)
    parser.add_argument("--perturbation-std", type=float, default=Phase2Config.perturbation_std)
    parser.add_argument("--ablation-fraction", type=float, default=Phase2Config.ablation_fraction)
    parser.add_argument("--enable-generation-bridge", action=argparse.BooleanOptionalAction, default=Phase2Config.enable_generation_bridge)
    parser.add_argument("--generation-eval-batches", type=int, default=Phase2Config.generation_eval_batches)
    parser.add_argument("--generation-sample-count", type=int, default=Phase2Config.generation_sample_count)
    parser.add_argument("--generation-max-length", type=int, default=Phase2Config.generation_max_length)
...
def build_forward_cache(hidden_states, attention_mask, config: Phase2Config) -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    metadata: dict[str, Any] = {"conditions": {}, "calibration": {}}
    ...
    for condition in config.forward_conditions:
        ...
        for stage_index, target_tokens in enumerate(config.stage_tokens, 1):
            ...
            if condition == "average_pool":
                ...
                sigma = config.sigma_values[stage_index - 1]
            ...
            if condition == "gaussian_noise":
                metadata["conditions"][condition][name]["noise_sigma"] = config.sigma_values[stage_index - 1]
            else:
                metadata["conditions"][condition][name]["noise_sigma"] = None

            if condition == "average_pool":
                upsampled = interpolate_tokens(latent, max_length)
                average_initial_losses[name] = float(functional.mse_loss(upsampled.float(), hidden.float()).item())
...
def evaluate_phase2_gates(metrics: dict[str, Any]) -> dict[str, Any]:
    results = metrics.get("results", {})
    required = {"average_pool", "strided_select", "random_select", "gaussian_noise"}
    p2_g1 = required.issubset(set(results))

    average_pool = results.get("average_pool", {})
    random_select = results.get("random_select", {})
    gaussian_noise = results.get("gaussian_noise", {})
    strided_select = results.get("strided_select", {})
    ...
    p2_g2 = bool(average_pool and random_select and (avg_pool_mse < random_mse or avg_pool_cos > random_cos))
    ...
    p2_g3 = bool(average_pool and gaussian_noise and (avg_pool_mse < gaussian_mse or (avg_delta_nll != 0.0 and avg_delta_nll < gaussian_delta_nll)))
    ...
    p2_g4 = len(avg_mses) >= 2 and monotonic_increasing(avg_mses) and monotonic_decreasing(avg_cosines)
    ...
    p2_g5 = sum(1 for item in latent_use_signals if item) >= 2

    strided_delta_nll = average_stage_metric(strided_select, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    random_delta_nll = average_stage_metric(random_select, ("final", "generation_bridge", "delta_token_nll_vs_h0"))
    compression_delta = min(value for value in (avg_delta_nll, strided_delta_nll) if isinstance(value, float)) if (average_pool or strided_select) else 0.0
    p2_g6 = bool(random_select and (compression_delta < random_delta_nll))

    gates = {
        "P2-G1": {"pass": p2_g1, "reason": "all four forward conditions ran on the same split"},
        "P2-G2": {"pass": p2_g2, "reason": "average_pool beats random_select on reconstruction MSE or cosine"},
        "P2-G3": {"pass": p2_g3, "reason": "average_pool beats gaussian_noise on reconstruction or token NLL"},
        "P2-G4": {"pass": p2_g4, "reason": "average_pool stages become harder monotonically"},
        "P2-G5": {"pass": p2_g5, "reason": "latent perturbation, ablation, or swap tests show latent use"},
        "P2-G6": {"pass": p2_g6, "reason": "compression condition has lower delta token NLL than random_select"},
    }
    gates["overall_pass"] = all(value["pass"] for key, value in gates.items() if key.startswith("P2-"))
    gates["diagnostics"] = {
...
def write_summary(output_dir: Path, metrics: dict[str, Any]) -> None:
    ...
    for gate_name, gate in metrics["gates"].items():
        if gate_name == "overall_pass" or gate_name == "diagnostics":
            continue
        lines.append(f"| {gate_name} | `{gate['pass']}` | {gate['reason']} |")
    lines.extend(["", f"- Overall pass: `{metrics['gates']['overall_pass']}`", "", "## Condition Results", ""])
    for condition, condition_results in metrics["results"].items():
        ...
        for stage_name, stage_result in condition_results.items():
            ...
            lines.append(
                ...
                f"{latent_use['ablation_delta_mse']:.6f} | {latent_use['swap_delta_mse']:.6f} | {delta_text} |"
            )
        lines.append("")
    lines.extend(["## Gaussian Calibration", "", "```json", json.dumps(metrics["forward_metadata"]["calibration"], indent=2), "```", ""])
    (output_dir / "summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def run_phase2(config: Phase2Config) -> dict[str, Any]:
    ...
    metrics["gates"] = evaluate_phase2_gates(metrics)
