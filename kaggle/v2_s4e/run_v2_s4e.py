"""LACE V2 S4e shared-condition semantic span expansion.

S4e keeps S4d's gap/span expansion task but removes the separate-model
confound. One shared reverse model receives a condition embedding that says
which schedule/control produced the current state. The experiment asks whether
importance skeleton content still beats random, same-position random,
wrong-document, position-only, and no-anchor controls when all conditions share
the same parameters. It also elevates generated span content/entity metrics
before any open-ended generation scale-up.
"""
...
class V2S4eConfig:
    ...
    output_dir: str = "/kaggle/working/lace_v2_s4e"
...
def parse_args() -> V2S4eConfig:
    parser = argparse.ArgumentParser(description="Run LACE V2 S4e shared-condition semantic span expansion.")
    parser.add_argument("--model-name", default=V2S4eConfig.model_name)
    parser.add_argument("--max-train-samples", type=int, default=V2S4eConfig.max_train_samples)
    parser.add_argument("--max-eval-samples", type=int, default=V2S4eConfig.max_eval_samples)
    parser.add_argument("--max-length", type=int, default=V2S4eConfig.max_length)
    parser.add_argument("--target-max-length", type=int, default=V2S4eConfig.target_max_length)
    parser.add_argument("--max-span-length", type=int, default=V2S4eConfig.max_span_length)
    parser.add_argument("--skeleton-batch-size", type=int, default=V2S4eConfig.skeleton_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=V2S4eConfig.train_batch_size)
    parser.add_argument("--eval-batch-size", type=int, default=V2S4eConfig.eval_batch_size)
    parser.add_argument("--reverse-epochs", type=int, default=V2S4eConfig.reverse_epochs)
    parser.add_argument("--learning-rate", type=float, default=V2S4eConfig.learning_rate)
    parser.add_argument("--use-amp", action=argparse.BooleanOptionalAction, default=V2S4eConfig.use_amp)
    parser.add_argument("--d-model", type=int, default=V2S4eConfig.d_model)
    parser.add_argument("--num-heads", type=int, default=V2S4eConfig.num_heads)
    parser.add_argument("--encoder-layers", type=int, default=V2S4eConfig.encoder_layers)
    parser.add_argument("--decoder-layers", type=int, default=V2S4eConfig.decoder_layers)
    parser.add_argument("--dropout", type=float, default=V2S4eConfig.dropout)
    parser.add_argument("--max-grad-norm", type=float, default=V2S4eConfig.max_grad_norm)
    parser.add_argument("--no-repeat-ngram-size", type=int, default=V2S4eConfig.no_repeat_ngram_size)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", V2S4eConfig.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=V2S4eConfig.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=V2S4eConfig.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=V2S4eConfig.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=V2S4eConfig.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=V2S4eConfig.seed)
    parser.add_argument("--ratios", default=",".join(str(item) for item in DEFAULT_RATIOS))
    parser.add_argument("--gate-tolerance", type=float, default=V2S4eConfig.gate_tolerance)
    parser.add_argument("--sample-output-count", type=int, default=V2S4eConfig.sample_output_count)
    parser.add_argument("--min-text-words", type=int, default=V2S4eConfig.min_text_words)
...
def load_texts(config: V2S4eConfig) -> tuple[list[str], str]:
...
def build_schedule_payloads(config: V2S4eConfig, tokenizer, encoder, device: str, texts: list[str]) -> list[dict[str, Any]]:
...
class S4eSharedSpanReverseModel(nn.Module):
    def __init__(
        self,
        config: V2S4eConfig,
        vocab_size: int,
...
def train_model(config, tokenizer, device, train_examples, initial_embedding):
    model = S4eSharedSpanReverseModel(
        config,
    ...
    for epoch in range(config.reverse_epochs):
        for batch_index, raw_batch in enumerate(train_loader):
            ...
            if not torch.isfinite(loss):
                raise RuntimeError(
                    f"Non-finite loss for shared-condition model at epoch={epoch} batch={batch_index}: "
...
def evaluate_gates(
    ...
    gates = {
        "S4E-G-RUN": {"pass": required.issubset(available), "detail": sorted(available)},
        "S4E-G-LOSS-FINITE": {
            "pass": losses_finite,
            "detail": {name: item["eval_loss"] for name, item in teacher_metrics.items()},
        },
        "S4E-G-SHARED-CONDITION-MODEL": {
            "pass": len(required) > 1,
            "detail": {"shared_condition_model": True, "condition_ids": CONDITION_IDS},
        },
        "S4E-G-IMPORTANCE-BEATS-RANDOM": {
        ...
        "S4E-G-IMPORTANCE-BEATS-POSITION-ONLY": {
        ...
        "S4E-G-IMPORTANCE-BEATS-SAME-POSITION-RANDOM": {
        ...
        "S4E-G-SPAN-CONTENT-GAIN": {
            "pass": importance_teacher.get("delta_content_recall", 0.0)
            > S4D_BASELINES["span_content_recall"] + 0.002,
            "detail": {
                ...
                "delta_vs_s4d": delta(
                    ...
                    S4D_BASELINES["span_content_recall"],
                ),
            },
        },
        "S4E-G-SPAN-ENTITY-GAIN": {
            "pass": importance_teacher.get("entity_recall", 0.0) > S4D_BASELINES["span_entity_recall"] + 0.001,
            "detail": {
        ...
        "S4E-G-FINAL-NONREGRESSION": {
            "pass": importance.get("final_content_recall", 0.0) >= S4D_BASELINES["final_content_recall"] - tolerance
            and importance.get("final_entity_recall", 0.0) >= S4D_BASELINES["final_entity_recall"] - tolerance
            and importance.get("semantic_drift_proxy", 1.0) <= S4D_BASELINES["semantic_drift_proxy"] + tolerance
            and importance.get("repetition_rate", 1.0) <= S4D_BASELINES["repetition_rate"] + tolerance,
            "detail": {
        ...
        "S4E-G-ARTIFACT-CHECK": {
            "pass": importance_teacher.get("artifact_rate", 1.0)
            <= random_teacher.get("artifact_rate", 0.0) + tolerance,
            "detail": {
        ...
        "S4E-G-ROLLOUT-RANDOM-NONREGRESSION": {
        ...
        "S4E-G-BEST-IDENTIFIED": {
            "pass": bool(best_name),
            "detail": {"best_schedule": best_name, "best_score": scores.get(best_name, 0.0), "scores": scores},
        },
    }
    gates["overall_pass"] = bool(
        gates["S4E-G-RUN"]["pass"]
        and gates["S4E-G-LOSS-FINITE"]["pass"]
        and gates["S4E-G-SHARED-CONDITION-MODEL"]["pass"]
        and gates["S4E-G-IMPORTANCE-BEATS-RANDOM"]["pass"]
        and gates["S4E-G-IMPORTANCE-BEATS-POSITION-ONLY"]["pass"]
        and gates["S4E-G-IMPORTANCE-BEATS-SAME-POSITION-RANDOM"]["pass"]
        and gates["S4E-G-WRONG-DOC-DROPS"]["pass"]
        and gates["S4E-G-NO-ANCHOR-DROPS"]["pass"]
        and gates["S4E-G-SPAN-CONTENT-GAIN"]["pass"]
        and gates["S4E-G-SPAN-ENTITY-GAIN"]["pass"]
        and gates["S4E-G-FINAL-NONREGRESSION"]["pass"]
        and gates["S4E-G-ARTIFACT-CHECK"]["pass"]
    )
    gates["process_ready"] = bool(gates["S4E-G-RUN"]["pass"] and gates["S4E-G-LOSS-FINITE"]["pass"])
...
def render_summary(result: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"| `s5_ready` | `{str(gates.get('s5_ready')).lower()}` | S4e still uses constrained delta insertion, so this remains false. |",
            "",
    ...
    lines.extend(
        [
            ...
            "",
            "S4e is a shared-condition gap/span expansion experiment, not open-ended language generation.",
            "Every condition is trained in one model, so the key comparison is whether real semantic anchor content still beats same-position, wrong-document, position-only, and no-anchor controls under shared parameters.",
            "Span content/entity gains are compared to the S4d baseline because S4e is meant to improve the information carried by newly unmasked spans, not merely preserve the S4d rollout ordering.",
        ]
    )
    return "\n".join(lines) + "\n"


def run(config: V2S4eConfig) -> dict[str, Any]:
    ...
    model, losses = train_model(config, tokenizer, device, combined_train_examples, initial_embedding)
    training_losses["shared_condition_model"] = losses
    for schedule_name in config.schedules:
    ...
    result = {
        "run_info": {
            "phase": "v2_s4e",
            "experiment_name": "S4e-shared-condition semantic span expansion",
