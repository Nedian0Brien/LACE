"""LACE V2 S3a terminal diagnostic.

S3a diagnoses why S3's random terminal baseline was unexpectedly strong by
separating terminal content, same-position random content, position-only input,
predicted anchors, and gold-anchor oracle controls.
"""
...
class V2S3AConfig:
    ...
    output_dir: str = "/kaggle/working/lace_v2_s3a"
...
def parse_args() -> V2S3AConfig:
    parser = argparse.ArgumentParser(description="Run LACE V2 S3a terminal diagnostic.")
    parser.add_argument("--model-name", default=V2S3AConfig.model_name)
    parser.add_argument("--max-train-samples", type=int, default=V2S3AConfig.max_train_samples)
    parser.add_argument("--max-eval-samples", type=int, default=V2S3AConfig.max_eval_samples)
    parser.add_argument("--max-length", type=int, default=V2S3AConfig.max_length)
    parser.add_argument("--target-max-length", type=int, default=V2S3AConfig.target_max_length)
    parser.add_argument("--anchor-max-length", type=int, default=V2S3AConfig.anchor_max_length)
    parser.add_argument("--skeleton-batch-size", type=int, default=V2S3AConfig.skeleton_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=V2S3AConfig.train_batch_size)
    parser.add_argument("--eval-batch-size", type=int, default=V2S3AConfig.eval_batch_size)
    parser.add_argument("--anchor-epochs", type=int, default=V2S3AConfig.anchor_epochs)
    parser.add_argument("--reverse-epochs", type=int, default=V2S3AConfig.reverse_epochs)
    parser.add_argument("--learning-rate", type=float, default=V2S3AConfig.learning_rate)
    parser.add_argument("--d-model", type=int, default=V2S3AConfig.d_model)
    parser.add_argument("--num-heads", type=int, default=V2S3AConfig.num_heads)
    parser.add_argument("--encoder-layers", type=int, default=V2S3AConfig.encoder_layers)
    parser.add_argument("--decoder-layers", type=int, default=V2S3AConfig.decoder_layers)
    parser.add_argument("--dropout", type=float, default=V2S3AConfig.dropout)
    parser.add_argument("--max-grad-norm", type=float, default=V2S3AConfig.max_grad_norm)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", V2S3AConfig.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=V2S3AConfig.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=V2S3AConfig.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=V2S3AConfig.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=V2S3AConfig.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=V2S3AConfig.seed)
    parser.add_argument("--keep-ratio", type=float, default=V2S3AConfig.keep_ratio)
    parser.add_argument("--gate-tolerance", type=float, default=V2S3AConfig.gate_tolerance)
    parser.add_argument("--sample-output-count", type=int, default=V2S3AConfig.sample_output_count)
    parser.add_argument("--min-text-words", type=int, default=V2S3AConfig.min_text_words)
...
def load_texts(config: V2S3AConfig) -> tuple[list[str], str]:
...
def build_terminals(config: V2S3AConfig, tokenizer, encoder, device: str, texts: list[str]) -> list[dict[str, Any]]:
    encoded_rows: list[dict[str, Any]] = []
    with torch.no_grad():
...
class S3ASeq2SeqModel(nn.Module):
    def __init__(
        self,
        config: V2S3AConfig,
        vocab_size: int,
...
def make_base_examples(tokenizer, config: V2S3AConfig, texts: list[str], payloads: list[dict[str, Any]], indices: list[int]) -> list[dict[str, Any]]:
    examples = []
...
def evaluate_gates(metrics: dict[str, dict[str, float]], tolerance: float) -> dict[str, Any]:
    ...
    content_scores = {
        ...
        for name in ("attention_terminal", "idf_terminal")
        if name in scores
    }
    best_content_name = max(content_scores, key=content_scores.get) if content_scores else ""
    best_content_score = content_scores.get(best_content_name, 0.0)
    same_position_score = scores.get("same_position_random_terminal", 0.0)
    position_only_score = scores.get("position_only", 0.0)
    random_score = scores.get("random_terminal", 0.0)
    predicted_anchor_score = scores.get("random_terminal_predicted_anchor", 0.0)
    oracle_anchor_score = scores.get("random_terminal_gold_anchor_oracle", 0.0)
    losses_finite = all(math.isfinite(item.get("eval_loss", float("inf"))) for item in metrics.values())
    random_above_content = random_score >= best_content_score
    random_explained = (
        not random_above_content
        or same_position_score + tolerance >= random_score
        or position_only_score + tolerance >= random_score
    )
    gates = {
        "S3A-G-RUN": {"pass": required.issubset(available), "detail": sorted(available)},
        "S3A-G-LOSS-FINITE": {"pass": losses_finite, "detail": {name: item["eval_loss"] for name, item in metrics.items()}},
        "S3A-G-CONTENT-BEATS-SAME-POSITION": {
            "pass": best_content_score > same_position_score,
            "detail": {
                "best_content_condition": best_content_name,
                "best_content_score": best_content_score,
                "same_position_random_score": same_position_score,
            },
        },
        "S3A-G-CONTENT-BEATS-POSITION-ONLY": {
            "pass": best_content_score > position_only_score,
            "detail": {
                "best_content_condition": best_content_name,
                "best_content_score": best_content_score,
        ...
        "S3A-G-ORACLE-LIFT": {
            "pass": oracle_anchor_score > predicted_anchor_score + tolerance,
            "detail": {
                "oracle_anchor_score": oracle_anchor_score,
                "predicted_anchor_score": predicted_anchor_score,
                "tolerance": tolerance,
            },
        },
        "S3A-G-RANDOM-EXPLAINED": {
            "pass": random_explained,
            "detail": {
                "random_score": random_score,
                "best_content_score": best_content_score,
                "same_position_random_score": same_position_score,
                "position_only_score": position_only_score,
                "random_above_content": random_above_content,
                "tolerance": tolerance,
            },
        },
        "S3A-G-BEST-IDENTIFIED": {
            "pass": bool(best_name),
            "detail": {"best_condition": best_name, "best_score": scores.get(best_name, 0.0), "scores": scores},
        },
    }
    gates["overall_pass"] = bool(
        gates["S3A-G-RUN"]["pass"]
        and gates["S3A-G-LOSS-FINITE"]["pass"]
        and gates["S3A-G-CONTENT-BEATS-SAME-POSITION"]["pass"]
        and gates["S3A-G-CONTENT-BEATS-POSITION-ONLY"]["pass"]
        and gates["S3A-G-BEST-IDENTIFIED"]["pass"]
    )
    gates["diagnostic_ready"] = bool(gates["S3A-G-RUN"]["pass"] and gates["S3A-G-LOSS-FINITE"]["pass"])
...
def render_summary(result: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            f"| `overall_pass` | `{str(gates.get('overall_pass')).lower()}` | Core S3a content-control signal. |",
            f"| `diagnostic_ready` | `{str(gates.get('diagnostic_ready')).lower()}` | Whether the diagnostic comparison is interpretable. |",
            f"| `s4_ready` | `{str(gates.get('s4_ready')).lower()}` | S3a is diagnostic, so this remains false by default. |",
            "",
            "## Condition Metrics",
            "",
    ...
    lines.extend(
        [
            ...
            "",
            "S3a is a terminal diagnostic, not an open-ended generation result.",
...
def run(config: V2S3AConfig) -> dict[str, Any]:
    ...
    for condition_name in config.conditions:
        terminal_key = terminal_key_for_condition(condition_name)
        train_anchor_predictions = anchor_runs.get(terminal_key, {}).get("train_predictions", {})
    ...
    result = {
        "run_info": {
            "phase": "v2_s3a",
            "experiment_name": "S3a-terminal diagnostic",
