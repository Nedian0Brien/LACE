...
S5는 [experiments/s5-semantic-plan-bridge.md](./experiments/s5-semantic-plan-bridge.md)에 기록했다. S5는 semantic skeleton과 surface span 사이에 content/entity plan 중간 표현을 넣는 실험이었다. `oracle_plan_schedule`은 span content recall 0.4144, entity recall 0.1191, artifact rate 0.1699로 S4g의 span collapse를 크게 회복했고, rollout score도 1.4027로 no-plan, random-plan, wrong-document, position-only control을 모두 이겼다. 하지만 `predicted_plan_schedule`은 plan recall 0.0146으로 random plan 0.0459보다 낮았고, rollout score도 no-plan과 거의 같았다.

현재 다음 단계는 open-ended generation이 아니다. S5는 "올바른 semantic plan이 있으면 span 생성이 살아난다"는 upper-bound는 확인했지만, "모델이 skeleton/anchor에서 그 plan을 스스로 찾는다"는 것은 확인하지 못했다. 따라서 heuristic planner는 연구 대상이 아니라 smoke/control/ablation으로만 두고, S5 내부의 primary condition은 learned semantic planner로 교체한다. 다음 확인점은 learned predicted plan rollout이 no-plan/random/wrong-document plan보다 좋아지는지다.
