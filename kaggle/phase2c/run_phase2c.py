"""LACE Phase 2C Kaggle runner.

Phase 2C probes whether explicit positional encoding helps average-pooling
compression expand back into decoder-readable hidden trajectories.

The script keeps Phase 2B's calibrated Gaussian, h0 decoder control, and
token-head bridge, but adds fixed sinusoidal positional conditioning to selected
expanders plus a position-only control. This separates three possibilities:

    1. average pooling lacks content information,
    2. average pooling has content information but lacks a usable position
       scaffold during reverse expansion,
    3. positional priors alone explain any apparent improvement.
"""
...
class Phase2CConfig:
    ...
    output_dir: str = "/kaggle/working/lace_phase2c"
    ...
    wrong_position_shift: int = 17


Phase2Config = Phase2CConfig
...
def base_condition_name(condition: str) -> str:
    if condition == "average_pool_positional":
        return "average_pool"
    if condition == "gaussian_noise_positional":
...
def condition_uses_positional_encoding(condition: str) -> bool:
    return condition in {"average_pool_positional", "gaussian_noise_positional", "position_only"}


def condition_is_position_only(condition: str) -> bool:
    return condition == "position_only"
...
def parse_args() -> Phase2CConfig:
    parser = argparse.ArgumentParser(description="Run LACE Phase 2C positional encoding probe.")
    parser.add_argument("--model-name", default=Phase2CConfig.model_name)
    parser.add_argument("--max-samples", type=int, default=Phase2CConfig.max_samples)
    parser.add_argument("--max-length", type=int, default=Phase2CConfig.max_length)
    parser.add_argument("--encode-batch-size", type=int, default=Phase2CConfig.encode_batch_size)
    parser.add_argument("--train-batch-size", type=int, default=Phase2CConfig.train_batch_size)
    parser.add_argument("--epochs", type=int, default=Phase2CConfig.epochs)
    parser.add_argument("--learning-rate", type=float, default=Phase2CConfig.learning_rate)
    parser.add_argument("--weight-decay", type=float, default=Phase2CConfig.weight_decay)
    parser.add_argument("--val-fraction", type=float, default=Phase2CConfig.val_fraction)
    parser.add_argument("--output-dir", default=os.environ.get("LACE_OUTPUT_DIR", Phase2CConfig.output_dir))
    parser.add_argument("--input-text-file", default=os.environ.get("LACE_INPUT_TEXT_FILE"))
    parser.add_argument("--use-hf-dataset", action=argparse.BooleanOptionalAction, default=Phase2CConfig.use_hf_dataset)
    parser.add_argument("--hf-dataset-name", default=Phase2CConfig.hf_dataset_name)
    parser.add_argument("--hf-dataset-config", default=Phase2CConfig.hf_dataset_config)
    parser.add_argument("--hf-dataset-split", default=Phase2CConfig.hf_dataset_split)
    parser.add_argument("--seed", type=int, default=Phase2CConfig.seed)
    parser.add_argument("--stage-tokens", default="64,32,16")
    parser.add_argument(
        "--forward-conditions",
        default="average_pool,average_pool_positional,position_only,random_select,gaussian_noise,gaussian_noise_positional",
    )
    ...
    parser.add_argument("--use-matched-gaussian", action=argparse.BooleanOptionalAction, default=Phase2CConfig.use_matched_gaussian)
    parser.add_argument("--lambda-cos", type=float, default=Phase2CConfig.lambda_cos)
    parser.add_argument("--lambda-var", type=float, default=Phase2CConfig.lambda_var)
    parser.add_argument("--perturbation-std", type=float, default=Phase2CConfig.perturbation_std)
    parser.add_argument("--ablation-fraction", type=float, default=Phase2CConfig.ablation_fraction)
    parser.add_argument("--enable-generation-bridge", action=argparse.BooleanOptionalAction, default=Phase2CConfig.enable_generation_bridge)
    parser.add_argument("--generation-eval-batches", type=int, default=Phase2CConfig.generation_eval_batches)
    parser.add_argument("--generation-sample-count", type=int, default=Phase2CConfig.generation_sample_count)
    parser.add_argument("--generation-max-length", type=int, default=Phase2CConfig.generation_max_length)
    parser.add_argument("--enable-token-head-bridge", action=argparse.BooleanOptionalAction, default=Phase2CConfig.enable_token_head_bridge)
    parser.add_argument("--token-head-epochs", type=int, default=Phase2CConfig.token_head_epochs)
    parser.add_argument("--token-head-learning-rate", type=float, default=Phase2CConfig.token_head_learning_rate)
    parser.add_argument("--token-head-train-batches", type=int, default=Phase2CConfig.token_head_train_batches)
    parser.add_argument("--token-head-eval-batches", type=int, default=Phase2CConfig.token_head_eval_batches)
    parser.add_argument("--strict-gate-min-evidence", type=int, default=Phase2CConfig.strict_gate_min_evidence)
    parser.add_argument("--positional-feature-size", type=int, default=Phase2CConfig.positional_feature_size)
    parser.add_argument("--wrong-position-shift", type=int, default=Phase2CConfig.wrong_position_shift)
    ...
    for condition in config.forward_conditions:
        ...
        for stage_index, target_tokens in enumerate(config.stage_tokens, 1):
            ...
            metadata["conditions"][condition][name] = {
                ...
                "position_mode": "sinusoidal" if condition_uses_positional_encoding(condition) else "learned_bias_only",
...
def sinusoidal_position_features(max_length: int, feature_size: int):
    ...
    features = torch.zeros(max_length, feature_size, dtype=torch.float32)
        ...
        def __init__(self) -> None:
            ...
            if self.use_positional_encoding:
                self.register_buffer(
                    "sinusoidal_position",
                    sinusoidal_position_features(max_length, positional_feature_size).unsqueeze(0),
                    persistent=False,
                )
    ...
    with torch.no_grad():
        for latents, hidden_states, attention_mask, input_ids, sample_indices in loader:
            ...
            if expander_uses_positional_encoding(expander):
                shifted_reconstruction = expander(latents, position_shift=config.wrong_position_shift)
                shifted_metrics = reconstruction_metrics(hidden_states, shifted_reconstruction, attention_mask)
                wrong_position_delta_values.append(float(shifted_metrics["mse"]) - base_mse)
...
evaluate_phase2_gates = evaluate_phase2c_gates
    ...
    for condition, condition_results in metrics["results"].items():
        lines.extend(
            [
                f"### {condition}",
                "",
                "| Stage | Pos enc | Val loss | MSE | Cosine | Var ratio | Rel sens | Abl delta | Swap delta | Wrong-pos dMSE | Decoder dNLL | Head dNLL | Meaningful gen |",
                "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
            ]
        )
        for stage_name, stage_result in condition_results.items():
            ...
            lines.append(
                f"| {stage_name} | `{stage_result['use_positional_encoding']}` | {stage_result['validation']['final_loss']:.6f} | {final['mse']:.6f} | "
...
def run_phase2c(config: Phase2Config) -> dict[str, Any]:
    ...
    metrics["gates"] = evaluate_phase2c_gates(metrics)
...
run_phase2 = run_phase2c


def main() -> None:
    config = parse_args()
    metrics = run_phase2c(config)
